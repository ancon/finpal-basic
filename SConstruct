...
'''
    ...
    install         Default target and copies it to build/FinPald (default)
...
GCC 5: If the gcc toolchain is used, gcc version 5 or better is required. On
    linux distros that ship with gcc 4 (ubuntu < 15.10), FinPald will force gcc
    to use gcc4's ABI (there was an ABI change between versions). This allows us
    to use the package manager to install FinPald dependencies. It also means if
...
Clang on linux: Clang cannot use the new gcc 5 ABI (clang does not know about
    ...
    link FinPald in this scenario (use the `--static` with scons), as dynamic
...
if (not platform.machine().endswith('64')):
    print('Warning: Detected {} architecture. FinPald requires a 64-bit OS.'.format(
          platform.machine()));

sys.path.append(os.path.join('src', 'mtchain', 'beast', 'site_scons'))
sys.path.append(os.path.join('src', 'mtchain', 'site_scons'))
...
AddOption('--gmssl', dest='gmssl', action='store_true',
          help='Use gmssl instead of openssl and add sm2 encryption support')

AddOption('--ipfs', dest='ipfs', action='store_true',
          help='Enable ipfs to support file upload/download')
...
UNITY_BUILD_DIRECTORY = 'src/mtchain/unity/'
...
def check_openssl():
    ...
    if parse_time(d) < parse_time(build_time):
        raise Exception('Your openSSL was built on %s; '
                        'FinPald needs a version built on or after %s.'
                        % (line, build_time))
...
def use_gcc4_abi(cc_cmd):
    if os.getenv('MTCHAIND_OLD_GCC_ABI'):
        return True
    gcc_ver = ''
    ubuntu_ver = None
    try:
...
def enable_gmssl ():
    return GetOption('gmssl')

def enable_ipfs ():
    return GetOption('ipfs')
...
#           print("machine = {}".format(platform.machine()))
            ...
            if enable_gmssl():
                for l in ['ssl', 'crypto']:
                    f = 'lib/linux/%s/lib%s.a' % (platform.machine(), l)
                    if not os.path.exists(f):
                        mylibs = []
                        break
                    mylibs.append(f)
            else:
                env.Append(CPPDEFINES=['OPENSSL_NO_SM2'])

            if mylibs:
                for l in mylibs:
                    env.Append(LIBS=[File(l)])
                env.Append(CPPPATH=[os.path.join('lib', 'include', 'gmssl')])
            else:
                libs.append('openssl')

            link_static = should_link_static()
            for l in libs: #['openssl', 'protobuf']:
            ...
            mylibs = []
            if enable_ipfs():
                mylibs.append('ipfs-api')
                env.Append(LIBS=['curl'])
                env.Append(CPPDEFINES=['IPFS_ENABLE'])

            if mylibs:
                env.Append(CPPPATH=[os.path.join('lib', 'include')])
                for l in mylibs:
                    f = 'lib/linux/%s/lib%s.a' % (platform.machine(), l)
                    env.Append(LIBS=[File(f)])
        ...
        if Beast.system.linux:
            lua_lib = 'src/slua/lib/linux/liblua5.2.a'
        elif Beast.system.windows:
            lua_lib = 'src/slua/lib/windows/lua52.lib'
	else:
	    lua_lib = 'lua5.2'

        if (os.path.exists(lua_lib)):
	    env.Append(LIBS=[File(lua_lib)])
	else:
	    env.Replace(LINKCOM=env['LINKCOM'] + " -Wl,-Bstatic -l" + lua_lib + " -Wl,-Bdynamic")
...
base = Environment(
    toolpath=[os.path.join ('src', 'mtchain', 'beast', 'site_scons', 'site_tools')],
...
base.Append(CPPPATH=[
    ...
    os.path.join('src', 'slua','include'),
    os.path.join('src', 'sqlite'),
    ])
...
for source in [
    'src/mtchain/proto/mtchain.proto',
    ]:
    base.Protoc([],
        source,
...
def get_soci_sources(style):
    ...
    append_sources(result,
                   'src/mtchain/unity/soci.cpp',
                   CPPPATH=cpp_path)
    if style == 'unity':
        append_sources(result,
                       'src/mtchain/unity/soci_mtchain.cpp',
                       CPPPATH=cpp_path)
    return result
...
def get_common_sources(toolchain):
    ...
    append_sources(
        result,
        'src/mtchain/unity/secp256k1.cpp',
...
def get_classic_sources(toolchain):
    ...
    append_sources(
        result,
        *list_sources('src/mtchain/core', '.cpp'),
        CPPPATH=[
            'src/soci/src/core',
            'src/sqlite']
    )
    append_sources(result, *list_sources('src/mtchain/beast/clock', '.cpp'))
    append_sources(result, *list_sources('src/mtchain/beast/container', '.cpp'))
    append_sources(result, *list_sources('src/mtchain/beast/insight', '.cpp'))
    append_sources(result, *list_sources('src/mtchain/beast/net', '.cpp'))
    append_sources(result, *list_sources('src/mtchain/beast/utility', '.cpp'))
    append_sources(result, *list_sources('src/mtchain/app', '.cpp'))
    append_sources(result, *list_sources('src/mtchain/basics', '.cpp'))
    append_sources(result, *list_sources('src/mtchain/conditions', '.cpp'))
    append_sources(result, *list_sources('src/mtchain/crypto', '.cpp'))
    append_sources(result, *list_sources('src/mtchain/json', '.cpp'))
    append_sources(result, *list_sources('src/mtchain/ledger', '.cpp'))
    append_sources(result, *list_sources('src/mtchain/legacy', '.cpp'))
    append_sources(result, *list_sources('src/mtchain/net', '.cpp'))
    append_sources(result, *list_sources('src/mtchain/overlay', '.cpp'))
    append_sources(result, *list_sources('src/mtchain/peerfinder', '.cpp'))
    append_sources(result, *list_sources('src/mtchain/protocol', '.cpp'))
    append_sources(result, *list_sources('src/mtchain/rpc', '.cpp'))
    append_sources(result, *list_sources('src/mtchain/shamap', '.cpp'))
    append_sources(result, *list_sources('src/mtchain/server', '.cpp'))
    ...
    append_sources(
        result,
        *(list_sources('src/mtchain/nodestore', '.cpp') + list_sources('src/test/nodestore', '.cpp')),
        CPPPATH=[
...
def get_unity_sources(toolchain):
    ...
    append_sources(
        result,
        'src/mtchain/beast/unity/beast_insight_unity.cpp',
        'src/mtchain/beast/unity/beast_net_unity.cpp',
        'src/mtchain/beast/unity/beast_utility_unity.cpp',
        'src/mtchain/unity/app_ledger.cpp',
        'src/mtchain/unity/app_main.cpp',
        'src/mtchain/unity/app_misc.cpp',
        'src/mtchain/unity/app_paths.cpp',
        'src/mtchain/unity/app_tx.cpp',
        'src/mtchain/unity/conditions.cpp',
        'src/mtchain/unity/core.cpp',
        'src/mtchain/unity/basics.cpp',
        'src/mtchain/unity/crypto.cpp',
        'src/mtchain/unity/ledger.cpp',
        'src/mtchain/unity/net.cpp',
        'src/mtchain/unity/overlay.cpp',
        'src/mtchain/unity/peerfinder.cpp',
        'src/mtchain/unity/json.cpp',
        'src/mtchain/unity/protocol.cpp',
        'src/mtchain/unity/rpcx.cpp',
        'src/mtchain/unity/shamap.cpp',
        'src/mtchain/unity/server.cpp',
    ...
    append_sources(
        result,
        'src/mtchain/unity/nodestore.cpp',
    ...
    for toolchain in all_toolchains:
        for variant in variants:
            ...
            object_builder.add_source_files(
                'src/mtchain/beast/unity/beast_hash_unity.cpp',
                'src/mtchain/unity/beast.cpp',
                'src/mtchain/unity/lz4.c',
                'src/mtchain/unity/protobuf.cpp',
                'src/mtchain/unity/mtchain.proto.cpp',
                'src/mtchain/unity/resource.cpp',
		'src/slua/lsqlite3.c',
                **cc_flags
            )
            ...
            object_builder.add_source_files(
                'src/mtchain/unity/ed25519_donna.c',
                CPPPATH=[
                    'src/ed25519-donna',
                ]
            )
            ...
            if toolchain == "clang" and Beast.system.osx:
                object_builder.add_source_files('src/mtchain/unity/beastobjc.mm')

            target = env.Program(
                target=os.path.join(variant_dir, 'FinPald'),
...
vcxproj = base.VSProject(
    os.path.join('Builds', 'VisualStudio2015', 'BithassD'),
    source = [],
    VSPROJECT_ROOT_DIRS = [
        'build/',
        'src/beast/extras',
...
# Build the list of FinPald source files that hold unit tests
