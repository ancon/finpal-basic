    ...
    : public beast::unit_test::suite
{
    ...
    testFlags()
    {
        ...
        env.trust(USD(100), alice, bob, carol);
        {
            ...
            env(offer(alice, M(5), USD(5)));

            // Now freeze gw.
            env(fset (gw, asfGlobalFreeze));
            env.close();
            env(offer(alice, M(4), USD(5)), ter(tecFROZEN));
            env.close();
        ...
        }
        {
            ...
            // carol's M balance should be her holdings minus her reserve.
            auto const carolsM = accountHolds (*env.closed(), carol,
                mCurrency(), mAccount(), fhZERO_IF_FROZEN, env.journal);
            // carol's M balance:              10000
            // base reserve:                      -200
            // 1 trust line times its reserve: 1 * -50
            //                                 -------
            // carol's available balance:         9750
            BEAST_EXPECT(carolsM == M(9750));

            // carol should be able to spend *more* than her M balance on
            // a fee by eating into her reserve.
            env(noop(carol), fee(carolsM + M(10)));
            env.close();

            // carol's M balance should now show as zero.
            BEAST_EXPECT(M(0) == accountHolds (*env.closed(),
                carol, mCurrency(), gw, fhZERO_IF_FROZEN, env.journal));
        }
        {
            // accountFunds().
...
//      static_assert (64 >= (2 * DIR_NODE_MAX), "");
        ...
        for (char b = 'A'; b <= 'D'; ++b)
        {
            for (char c = 'A'; c <= 'P'; ++c)
            {
                ...
                env.close();
                env(offer(alice, currency(50), M(10)));
                env(offer(becky, M(10), currency(50)));
                env.close();
            }
        }
        ...
        env.close();
        env(offer(alice, lastCurrency(50), M(10)));
        env(offer(becky, M(10), lastCurrency(50)));
        env.close();
...
BEAST_DEFINE_TESTSUITE(View,ledger,mtchain);
BEAST_DEFINE_TESTSUITE(GetAmendments,ledger,mtchain);
BEAST_DEFINE_TESTSUITE(DirIsEmpty, ledger,mtchain);
