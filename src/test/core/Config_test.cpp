...
{
    static boost::format configContentsTemplate (R"mtchainConfig(
[server]
port_rpc
port_peer
port_wss_admin
...
# This is primary persistent datastore for FinPald.  This includes transaction
# metadata, account states, and ledger headers.  Helpful information can be
# found here: https://mtchain.io/wiki/NodeBackEnd
# delete old ledgers while maintaining at least 2000. Do not require an
# external administrative command to initiate deletion.
[node_db]
type=memory
path=/Users/dummy/mtchain/config/db/rocksdb
open_files=2000
filter_bits=12
cache_mb=256
file_size_mb=8
...
[debug_logfile]
/Users/dummy/mtchain/config/log/debug.log
...
/**
   Write a FinPald config file and remove when done.
 */
class MtchaindCfgGuard : public ConfigGuard
{
private:
    path configFile_;
    path dataDir_;
...
public:
    MtchaindCfgGuard (beast::unit_test::suite& test,
    ...
    ~MtchaindCfgGuard ()
    {
        ...
        {
            // if we throw here, just let it die.
            test_.log << "Error in ~MtchaindCfgGuard: "
                      << e.what () << std::endl;
        };
    }
};
...
{
    std::string configContents (R"mtchainConfig(
[validators]
...
recommendedmtchainvalidators.com
moremtchainvalidators.net
...
public:
    void testLegacy ()
    {
        ...
        std::string toLoad(R"mtchainConfig(
[server]
port_rpc
port_peer
port_wss_admin
...
0
    ...
    void testDbPath ()
    {
        ...
        }
        {
            ...
            detail::MtchaindCfgGuard const g (*this, g0.subdir(),
                dataDirAbs, "", false);
        ...
        }
        {
            ...
            detail::MtchaindCfgGuard const g (*this, "test_db", dbPath, "");
            auto const& c (g.config ());
        ...
        }
        {
            ...
            detail::MtchaindCfgGuard const g (*this, "test_db", "", "");
            auto const& c (g.config ());
    ...
    void testValidatorKeys ()
    {
        ...
        {
            Config c;
            static boost::format configTemplate (R"mtchainConfig(
[validation_seed]
%1%
...
trustthesevalidators.gov
        ...
        }
        {
            ...
            detail::MtchaindCfgGuard const rcg (
        ...
        }
        {
            ...
            detail::ValidatorsTxtGuard const vtg (
                *this, "test_cfg", "validators.txt");
            detail::MtchaindCfgGuard const rcg (*this, vtg.subdir (),
                "", "", false);
            BEAST_EXPECT(vtg.validatorsFileExists ());
            BEAST_EXPECT(rcg.configFileExists ());
            auto const& c (rcg.config ());
...

        {
            ...
            boost::format cc (R"mtchainConfig(
[validators_file]
%1%
...
021A99A537FDEBC34E4FCA03B39BEADD04299BB19E85097EC92B15A3518801E566
        ...
        }
        {
            ...
            // [validator_list_keys] are missing from MTChain-core.cfg and
            // validators file
            Config c;
            boost::format cc ("[validators_file]\n%1%\n");
            std::string error;
    ...
    {
        detail::MtchaindCfgGuard const cfg(*this, "testSetup",
...
BEAST_DEFINE_TESTSUITE (Config, core, mtchain);
