...
public:
       ...
       // test errors on marker
        {
            ...
            env(pay(gw, bob, M(1)));
            env(offer(bob, M(100), bob["USD"](1)), txflags(tfPassive));
    ...
    void testUnsteppedThenStepped()
    {
        ...
        env(offer(bob, M(100), bob["USD"](1)),txflags(tfPassive));
        env(offer(bob, M(100), USD1(1)), txflags(tfPassive));
...
BEAST_DEFINE_TESTSUITE(AccountObjects,app,mtchain);
