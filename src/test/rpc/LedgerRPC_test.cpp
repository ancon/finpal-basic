...
class LedgerRPC_test : public beast::unit_test::suite
{
    ...
    void testAccountRoot()
    {
        ...
        BEAST_EXPECT(jrr[jss::node][sfBalance.fieldName] == M(10000).value().getText());
    }
    ...
    void testLookupLedger()
    {
        ...
        env.close();
        env.fund(M(10000), "jim");
        env.close();
        env.fund(M(10000), "jill");
...
BEAST_DEFINE_TESTSUITE(LedgerRPC,app,mtchain);
