...
#include <mtchain/protocol/JsonFields.h>     // jss:: definitions
...
public:
    void test_noReserve()
    {
...

        {
            ...
            env(pay(env.master, alice, M(151)));
            env.close();
            env(smallSigners);
            env.close();
        ...
        }
        {
            ...
            env(pay(env.master, alice, M(350)));
            env.close();
            env(bigSigners);
            env.close();
...
//      env(signers(alice, 1, { { bogie, 0x10000} }), ter (temBAD_WEIGHT));
    ...
    void test_masterSigners()
    {
        ...
        env.fund(M(1000), alice, becky, cheri);
        env.close();
    ...
    void test_heterogeneousSigners()
    {
        ...
        env.fund(M(1000), alice, becky, cheri, daria);
        env.close();
    ...
    void test_txTypes()
    {
        ...
        env.fund(M(1000), alice, becky, zelda, gw);
        env.close();
...
BEAST_DEFINE_TESTSUITE(MultiSign, app, mtchain);
