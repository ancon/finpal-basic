...
class Freeze_test : public beast::unit_test::suite
{
    ...
    void testMTChainState()
    {
        testcase("MTChainState Freeze");
        ...
        env(offer(alice, M(500), G1["USD"](100)));
        env.close();
    ...
    testGlobalFreeze()
    {
        ...
        env(offer(G1, M(10000), G1["USD"](100)), txflags(tfPassive));
        env(offer(G1, G1["USD"](100), M(10000)), txflags(tfPassive));
        env(offer(A1, M(10000), G1["USD"](100)), txflags(tfPassive));
        env(offer(A2, G1["USD"](100), M(10000)), txflags(tfPassive));
        env.close();
...

        {
            ...
            env(offer(A3, G1["BTC"](1), M(1)));

            //    test: assets can be sold on the market
            env(offer(A4, M(1), G1["BTC"](1)));
...

        {
            ...
            env(offer(A3, G1["BTC"](1), M(1)), ter(tecFROZEN));

            //    test: assets can't be sold on the market
            env(offer(A4, M(1), G1["BTC"](1)), ter(tecFROZEN));
        }
    ...
    testOffersWhenFrozen()
    {
        ...
        env(offer(A3, M(1000), G1["USD"](1000)), txflags(tfPassive));
        env.close();

        // removal after successful payment
        //    test: make a payment with partially consuming offer
        env(pay(A2, G1, G1["USD"](1)), paths(G1["USD"]), sendmax(M(1)));
        env.close();
...
BEAST_DEFINE_TESTSUITE(Freeze, app, mtchain);
} //
