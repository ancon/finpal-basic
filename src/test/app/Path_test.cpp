...
rpf(jtx::Account const& src, jtx::Account const& dst, std::uint32_t num_src)
{
    ...
    jv[jss::command] = "mtchain_path_find";
...
public:
                    ...
                    boost::optional<Currency> const& saSrcCurrency = boost::none)
    {
        ...
        params[jss::command] = "mtchain_path_find";
...

        {
            ...
            std::tie(st, sa, da) = find_paths(env,
                "alice", "bob", Account("bob")["AUD"](-1),
                    boost::optional<STAmount>(M(100000000)));
            BEAST_EXPECT(st.empty());
            std::tie(st, sa, da) = find_paths(env,
                "alice", "bob", Account("bob")["USD"](-1),
                    boost::optional<STAmount>(M(100000000)));
            BEAST_EXPECT(sa == M(100));
    ...
    alternative_path_consume_both()
    {
        ...
        env.fund(M(10000), "alice", "bob", gw, gw2);
        env.trust(USD(600), "alice");
    ...
    alternative_paths_limit_returned_paths_to_best_quality()
    {
        ...
        auto const gw2 = Account("gateway2");
        auto const gw2_USD = gw2["USD"];
        env.fund(M(10000), "alice", "bob", "carol", "dan", gw, gw2);
        env(rate("carol", 1.1));
    ...
    void
    issues_path_negative_mtchain_client_issue_23_smaller()
    {
        testcase("path negative: mtchain-client issue #23: smaller");
        using namespace jtx;
        Env env(*this);
        env.fund(M(10000), "alice", "bob", "carol", "dan");
    ...
    void
    issues_path_negative_mtchain_client_issue_23_larger()
    {
        testcase("path negative: mtchain-client issue #23: larger");
        using namespace jtx;
        Env env(*this);
        env.fund(M(10000), "alice", "bob", "carol", "dan", "edward");
    ...
    // carol holds gateway AUD, sells gateway AUD for M
    // bob will hold gateway AUD
    // alice pays bob gateway AUD using M
    void
    via_offers_via_gateway()
    {
        ...
        env(offer("carol", M(50), AUD(50)));
        env(pay("alice", "bob", AUD(10)), sendmax(M(100)), paths(M));
    ...
    void path_find_01()
    {
        testcase("Path Find: M -> M and M -> IOU");
        using namespace jtx;
        Env env(*this);
        Account A1 {"A1"};
        Account A2 {"A2"};
        ...
        env.fund(M(100000), A1);
        env.fund(M(10000), A2);
        env.fund(M(1000), A3, G1, G2, G3, M1);
        env.close();
        ...
        env(offer(M1, M(10000), G3["ABC"](1000)));

        STPathSet st;
        STAmount sa, da;

        {
            auto const& send_amt = M(10);
            std::tie(st, sa, da) = find_paths(env, A1, A2, send_amt,
                boost::none, mCurrency());
    ...
    void path_find_02()
    {
        ...
        env(offer(M1, G3["ABC"](1000), M(10000)));
        ...
        BEAST_EXPECT(same(st, stpath(G3, IPE(mIssue()))));
    }
    ...
    void path_find_03()
    {
        ...
        env.fund(M(4999.999898), SRC);
        env.fund(M(10846.168060), GATEWAY_DST);
        env.fund(M(4291.430036), MONEY_MAKER_1);
        env.fund(M(106839.375770), MONEY_MAKER_2);
        env.fund(M(1240.997150), A1);
        env.fund(M(14115.046893), A2);
        env.fund(M(512087.883181), A3);
        env.close();
        ...
        env(offer(MONEY_MAKER_2, M(1), GATEWAY_DST["CNY"](1)));
        env(offer(MONEY_MAKER_2, GATEWAY_DST["CNY"](1), M(1)));
        env(offer(MONEY_MAKER_2, GATEWAY_DST["CNY"](318000), M(53000)));
        env(offer(MONEY_MAKER_2, M(209), MONEY_MAKER_2["CNY"](4.18)));
        env(offer(MONEY_MAKER_2, MONEY_MAKER_1["CNY"](990000), M(10000)));
        env(offer(MONEY_MAKER_2, MONEY_MAKER_1["CNY"](9990000), M(10000)));
        env(offer(MONEY_MAKER_2, GATEWAY_DST["CNY"](8870000), M(10000)));
        env(offer(MONEY_MAKER_2, M(232), MONEY_MAKER_2["CNY"](5.568)));
        env(offer(A2, M(2000), MONEY_MAKER_1["CNY"](66.8)));
        env(offer(A2, M(1200), GATEWAY_DST["CNY"](42)));
        env(offer(A2, MONEY_MAKER_1["CNY"](43.2), M(900)));
        env(offer(A3, MONEY_MAKER_1["CNY"](2240), M(50000)));
        ...
        std::tie(st, sa, da) = find_paths(env, SRC, GATEWAY_DST, send_amt,
            boost::none, mCurrency());
        BEAST_EXPECT(equal(da, send_amt));
        BEAST_EXPECT(equal(sa, M(288.571429)));
    ...
    void path_find_04()
    {
        ...
        env.fund(M(1000), G1BS, G2SW, A1, A2);
        env.fund(M(11000), M1);
        env.close();
        ...
        // can charge a fee when transactions mtchain across. Liquidity
    ...
    void path_find_05()
    {
        testcase("Path Find: non-M -> non-M, same currency");
        using namespace jtx;
        Env env(*this);
        Account A1 {"A1"};
        Account A2 {"A2"};
        ...
        env.fund(M(1000), A1, A2, A3, G1, G2, G3, G4);
        env.fund(M(10000), A4);
        env.fund(M(11000), M1, M2);
        env.close();
        ...
        env(offer(M2, M(10000), G2["HKD"](1000)));
        env(offer(M2, G1["HKD"](1000), M(10000)));
...

        {
            ...
            BEAST_EXPECT(same(st,
                ...
                stpath(G1, IPE(mIssue()), IPE(G2["HKD"]))
            ));
        }

        {
            //I4) M bridge" --
            //  Source -> AC -> OB to M -> OB from M -> AC -> Destination
            auto const& send_amt = A2["HKD"](10);
            std::tie(st, sa, da) = find_paths(env, A1, A2, send_amt,
                boost::none, G1["HKD"].currency);
            BEAST_EXPECT(equal(da, send_amt));
            ...
            BEAST_EXPECT(same(st,
                ...
                stpath(G1, IPE(mIssue()), IPE(G2["HKD"]), G2)
            ));
        }
    }
    ...
    void path_find_06()
    {
        ...
        env.fund(M(11000), M1);
        env.fund(M(1000), A1, A2, A3, G1, G2);
        env.close();
...
BEAST_DEFINE_TESTSUITE(Path,app,mtchain);
