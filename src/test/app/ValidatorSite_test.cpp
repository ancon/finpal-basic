...
/*
    ...
    Copyright 2019 ~ 2020 FinPal Alliance.
...
private:
    ...
    testConfigLoad ()
    {
        ...
        std::vector<std::string> cfgSites({
            "http://mtchain.io/",
            "http://mtchain.io/validators",
            "http://mtchain.io:8080/validators",
            "http://207.261.33.37/validators",
            "http://207.261.33.37:8080/validators",
            "https://mtchain.io/validators",
            "https://mtchain.io:443/validators"});
        ...
        std::vector<std::string> badSites(
            {"ftp://mtchain.io/validators"});
        BEAST_EXPECT(!trustedSites->load (badSites));

        badSites[0] = "wss://mtchain.io/validators";
        BEAST_EXPECT(!trustedSites->load (badSites));

        badSites[0] = "mtchain.io/validators";
        BEAST_EXPECT(!trustedSites->load (badSites));
    }
...
BEAST_DEFINE_TESTSUITE(ValidatorSite, app, mtchain);
