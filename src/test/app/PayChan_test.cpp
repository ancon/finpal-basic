...
struct PayChan_test : public beast::unit_test::suite
{
    ...
    {
        Serializer msg;
        serializePayChanAuthorization (msg, channel, authAmt.m ());
    ...
    testSimple ()
    {
        ...
        env (create (alice, bob, M (1000), settleDelay, pk));
        auto const chan = channel (*env.current (), alice, bob);
        BEAST_EXPECT (channelBalance (*env.current (), chan) == M (0));
        BEAST_EXPECT (channelAmount (*env.current (), chan) == M (1000));

        {
            auto const preAlice = env.balance (alice);
            env (fund (alice, chan, M (1000)));
            auto const feeDrops = env.current ()->fees ().base;
            BEAST_EXPECT (env.balance (alice) == preAlice - M (1000) - feeDrops);
        }

        auto chanBal = channelBalance (*env.current (), chan);
        auto chanAmt = channelAmount (*env.current (), chan);
        BEAST_EXPECT (chanBal == M (0));
        BEAST_EXPECT (chanAmt == M (2000));

        {
            // bad amounts (non-m, negative amounts)
            env (create (alice, bob, UM (1000), settleDelay, pk),
                ter (temBAD_AMOUNT));
            env (fund (alice, chan, UM (1000)),
                ter (temBAD_AMOUNT));
            env (create (alice, bob, M (-1000), settleDelay, pk),
                ter (temBAD_AMOUNT));
            env (fund (alice, chan, M (-1000)),
        ...
        // invalid account
        env (create (alice, "noAccount", M (1000), settleDelay, pk),
            ter (tecNO_DST));
        // can't create channel to the same account
        env (create (alice, alice, M (1000), settleDelay, pk),
             ter (temDST_IS_SRC));
        // invalid channel
        env (fund (alice, channel (*env.current (), alice, "noAccount"), M (1000)),
            ter (tecNO_ENTRY));
        // not enough funds
        env (create (alice, bob, M (10000), settleDelay, pk),
            ter (tecUNFUNDED));

        {
            // No signature claim with bad amounts (negative and non-m)
            auto const iou = UM (100).value ();
            auto const negM = M (-100).value ();
            auto const posM = M (100).value ();
            env (claim (alice, chan, iou, iou), ter (temBAD_AMOUNT));
            env (claim (alice, chan, posM, iou), ter (temBAD_AMOUNT));
            env (claim (alice, chan, iou, posM), ter (temBAD_AMOUNT));
            env (claim (alice, chan, negM, negM), ter (temBAD_AMOUNT));
            env (claim (alice, chan, posM, negM), ter (temBAD_AMOUNT));
            env (claim (alice, chan, negM, posM), ter (temBAD_AMOUNT));
        }
        {
            // No signature claim more than authorized
            auto const delta = M (500);
            auto const reqBal = chanBal + delta;
            auto const authAmt = reqBal + M (-100);
        ...
        }
        {
            ...
            auto const preBob = env.balance (bob);
            STAmount const authAmt = chanBal + M (500);
        ...
        env (fund (bob, chan, M (1000)), ter (tecNO_PERMISSION));
        BEAST_EXPECT (channelBalance (*env.current (), chan) == chanBal);
        BEAST_EXPECT (channelAmount (*env.current (), chan) == chanAmt);

        {
            // Wrong signing key
            auto const sig =
                signClaimAuth (bob.pk (), bob.sk (), chan, M (1500));
            env (claim (bob, chan, M (1500).value (), M (1500).value (),
                     Slice (sig), bob.pk ()),
                ter (temBAD_SIGNER));
            BEAST_EXPECT (channelBalance (*env.current (), chan) == chanBal);
            BEAST_EXPECT (channelAmount (*env.current (), chan) == chanAmt);
    ...
    testCancelAfter ()
    {
        ...
        auto const carol = Account ("carol");
        {
            ...
            auto const channelFunds = M (1000);
            env (create (
    ...
    testExpiration ()
    {
        ...
        env (fund (alice, chan, M (1),
            ...
            ter (temBAD_EXPIRATION));
        BEAST_EXPECT (!channelExpiration (*env.current (), chan));
        env (fund (alice, chan, M (1), NetClock::time_point{minExpiration}));
        env.close (minExpiration);
        // Try to extend the expiration after the expiration has already passed
        env (fund (
            alice, chan, M (1), NetClock::time_point{minExpiration + 1000s}));
        BEAST_EXPECT (!channelExists (*env.current (), chan));
    }
    ...
    testDisallowM ()
    {
        ...
        using namespace std::literals::chrono_literals;
        {
            // Create a channel where dst disallows M
            Env env (*this, features (featurePayChan));
            auto const alice = Account ("alice");
            auto const bob = Account ("bob");
            env.fund (M (10000), alice, bob);
            env (fset (bob, asfDisallowM));
            auto const pk = alice.pk ();
            auto const settleDelay = 3600s;
            auto const channelFunds = M (1000);
            env (create (alice, bob, channelFunds, settleDelay, pk),
        ...
        {
            // Claim to a channel where dst disallows M
            // (channel is created before disallow m is set)
            Env env (*this, features (featurePayChan));
            auto const alice = Account ("alice");
            auto const bob = Account ("bob");
            env.fund (M (10000), alice, bob);
    ...
    testMalformedPK ()
    {
        ...
        auto jv = create (alice, bob, M (1000), settleDelay, pk);
...
BEAST_DEFINE_TESTSUITE (PayChan, app, mtchain);
}  // test
}  //
