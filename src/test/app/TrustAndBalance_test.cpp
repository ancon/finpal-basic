...
class TrustAndBalance_test : public beast::unit_test::suite
{
    ...
    testPayNonexistent ()
    {
        ...
        Env env {*this};
        env (pay (env.master, "alice", M(1)), ter(tecNO_DST_INSUF_M));
        env.close();
    }
    ...
    testDirectMTChain ()
    {
        testcase ("Direct Payment, MTChain");
        using namespace test::jtx;
    ...
    testIndirectMultiPath (bool with_rate)
    {
        ...
        env.fund (M(10000), gw, amazon, alice, bob, carol);
        env.close();
...
BEAST_DEFINE_TESTSUITE (TrustAndBalance, app, mtchain);
