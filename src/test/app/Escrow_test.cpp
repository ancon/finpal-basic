...
struct Escrow_test : public beast::unit_test::suite
{
    ...
    testEnablement()
    {
        ...
        { // Escrow not enabled
            Env env(*this);
            env.fund(M(5000), "alice", "bob");
            env(lockup("alice", "bob", M(1000), env.now() + 1s), ter(temDISABLED));
        ...
        { // Escrow enabled
            ...
            env.fund(M(5000), "alice", "bob");
            env(lockup("alice", "bob", M(1000), env.now() + 1s));
            env.close();

            auto const seq = env.seq("alice");

            env(condpay("alice", "bob", M(1000),
    ...
    testFails()
    {
        ...
        env(condpay("alice", "carol", M(1000),
            makeSlice(cb1), env.now() + 1s),            ter(tecNO_DST));

        env.fund(M(5000), "carol");
        env(condpay("alice", "carol", M(1000),
        ...
        // Sending zero or no M:
        env (lockup("alice", "carol", M(0),
            env.now() + 1s),                            ter(temBAD_AMOUNT));
        env (lockup("alice", "carol", M(-1000),
            env.now() + 1s),                            ter(temBAD_AMOUNT));

        // Fail if neither CancelAfter nor FinishAfter are specified:
        {
            auto j1 = lockup("alice", "carol", M(1), env.now() + 1s);
            j1.removeMember ("FinishAfter");
            env (j1, ter(temBAD_EXPIRATION));

            auto j2 = condpay("alice", "carol", M(1), makeSlice(cb1), env.now() + 1s);
            j2.removeMember ("CancelAfter");
            env (j2, ter(temBAD_EXPIRATION));
        }

        // Fail if FinishAfter has already passed:
        env (lockup("alice", "carol", M(1), env.now() - 1s), ter (tecNO_PERMISSION));

        // Both CancelAfter and FinishAfter
        env(condpay("alice", "carol", M(1), makeSlice(cb1),
        ...
        env.fund (accountReserve + accountIncrement + M(50), "daniel");
        env(lockup("daniel", "bob", M(51), env.now() + 1s), ter (tecUNFUNDED));

        env.fund (accountReserve + accountIncrement + M(50), "evan");
        env(lockup("evan", "bob", M(50), env.now() + 1s),   ter (tecUNFUNDED));

        env.fund (accountReserve, "frank");
        env(lockup("frank", "bob", M(1), env.now() + 1s),   ter (tecINSUFFICIENT_RESERVE));

        // Respect the "asfDisallowM" account flag:
        env.fund (accountReserve + accountIncrement, "george");
        env(fset("george", asfDisallowM));
        env(lockup("bob", "george", M(10), env.now() + 1s),  ter (tecNO_TARGET));

        { // Specify incorrect sequence number
            env.fund (M(5000), "hannah");
            auto const seq = env.seq("hannah");
            env(lockup("hannah", "hannah", M(10), env.now() + 1s));
            env(finish ("hannah", "hannah", seq + 7),         ter (tecNO_TARGET));
        }

        { // Try to specify a condition for a non-conditional payment
            env.fund (M(5000), "ivan");
            auto const seq = env.seq("ivan");

            auto j = lockup("ivan", "ivan", M(10), env.now() + 1s);
    ...
    testLockup()
    {
        ...
        { // Unconditional
            ...
            env.fund(M(5000), "alice", "bob");
            auto const seq = env.seq("alice");
            env(lockup("alice", "alice", M(1000), env.now() + 1s));
            env.require(balance("alice", M(4000) - drops(10)));
        ...
        { // Conditional
            ...
            env.fund(M(5000), "alice", "bob");
            auto const seq = env.seq("alice");
            env(lockup("alice", "alice", M(1000), makeSlice(cb2), env.now() + 1s));
            env.require(balance("alice", M(4000) - drops(10)));
    ...
    testEscrowConditions()
    {
        ...
        { // Test cryptoconditions
            ...
            auto T = [&env](NetClock::duration const& d)
                { return env.now() + d; };
            env.fund(M(5000), "alice", "bob", "carol");
            auto const seq = env.seq("alice");
            BEAST_EXPECT((*env.le("alice"))[sfOwnerCount] == 0);
            env(condpay("alice", "carol", M(1000), makeSlice(cb1), T(S{1})));
            BEAST_EXPECT((*env.le("alice"))[sfOwnerCount] == 1);
            env.require(balance("alice", M(4000) - drops(10)));
            env.require(balance("carol", M(5000)));
            env(cancel("bob", "alice", seq),                                 ter(tecNO_PERMISSION));
            BEAST_EXPECT((*env.le("alice"))[sfOwnerCount] == 1);
            ...
            BEAST_EXPECT((*env.le("alice"))[sfOwnerCount] == 0);
            env.require(balance("carol", M(6000)));
        ...
        { // Test cancel when condition is present
            ...
            auto const seq = env.seq("alice");
            BEAST_EXPECT((*env.le("alice"))[sfOwnerCount] == 0);
            env(condpay("alice", "carol", M(1000), makeSlice(cb2), T(S{1})));
            env.close();
            env.require(balance("alice", M(4000) - drops(10)));
            // balance restored on cancel
            env(cancel("bob", "alice", seq));
            env.require(balance("alice", M(5000) - drops(10)));
...

        {
            ...
            env.close();
            auto const seq = env.seq("alice");
            env(condpay("alice", "carol", M(1000), makeSlice(cb3), T(S{1})));
            BEAST_EXPECT((*env.le("alice"))[sfOwnerCount] == 1);
        ...
        { // Test long & short conditions during creation
            ...
            env(finish("bob", "alice", seq,
                makeSlice(cb1), makeSlice(fb1)), fee(1500));
            env.require(balance("alice", M(4000) - drops(100)));
            env.require(balance("bob", M(5000) - drops(1500)));
            env.require(balance("carol", M(6000)));
        }
        ...
        { // Test empty condition during creation and
          // empty condition & fulfillment during finish
            ...
            env(condpay("alice", "carol", M(1000), {}, T(S{1})), ter(temMALFORMED));
        ...
        { // Test a condition other than PreimageSha256, which
          // would require a separate amendment
            ...
            env(condpay("alice", "carol", M(1000), makeSlice(cb), T(S{1})),
                ter(temMALFORMED));
        }
    }
    ...
    testMeta()
    {
        ...
        env.fund(M(5000), "alice", "bob", "carol");
        env(condpay("alice", "carol", M(1000), makeSlice(cb1), env.now() + 1s));
    ...
    void testConsequences()
    {
...

        {
            ...
            BEAST_EXPECT(conseq.potentialSpend == M(1000));
        }
...
BEAST_DEFINE_TESTSUITE(Escrow,app,mtchain);
