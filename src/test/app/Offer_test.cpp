...
{
    MAmount reserve(jtx::Env& env, std::uint32_t count)
    {
        ...
        -> jtx::PrettyAmount
    {
        ...
        return drops (dropsPerM<std::int64_t>::value * mAmount - feeDrops);
    };
    ...
    ledgerEntryState(jtx::Env & env, jtx::Account const& acct_a, jtx::Account const & acct_b, std::string const& currency)
    {
        ...
        jvParams[jss::FinPal_state][jss::currency] = currency;
        jvParams[jss::FinPal_state][jss::accounts] = Json::arrayValue;
        jvParams[jss::FinPal_state][jss::accounts].append(acct_a.human());
        jvParams[jss::FinPal_state][jss::accounts].append(acct_b.human());
...
public:
    void testRmFundedOffer ()
    {
        ...
        // be satisified. The first offer must be "taker gets" M. Old, broken
        // would remove the first "taker gets" m offer, even though the offer is
    ...
    void testCanceledOffer ()
    {
        ...
        BEAST_EXPECT(isOffer (env, alice, M (500), USD (100)));

        // cancel the offer above and replace it with a new offer
        env (offer (alice, M (300), USD (100)),
             json (jss::OfferSequence, firstOfferSeq),
             require (offers (alice, 1)));

        BEAST_EXPECT(isOffer (env, alice, M (300), USD (100)) &&
            !isOffer (env, alice, M (500), USD (100)));

        // Test canceling non-existent offer.
        env (offer (alice, M (400), USD (200)),
             json (jss::OfferSequence, firstOfferSeq),
             require (offers (alice, 2)));

        BEAST_EXPECT(isOffer (env, alice, M (300), USD (100)) &&
            isOffer (env, alice, M (400), USD (200)));

        // Test cancellation now with OfferCancel tx
        auto const nextOfferSeq = env.seq (alice);
        env (offer (alice, M (222), USD (111)),
            require (offers (alice, 3)));

        BEAST_EXPECT(isOffer (env, alice, M (222), USD (111)));
    ...
    void testMTinyPayment ()
    {
        testcase ("M Tiny payments");

        // Regression test for tiny m payments
        // In some cases, when the payment code calculates
        // the amount of m needed as input to an m->iou offer
        ...
        // 1) When we calculate the amount of input m needed for an offer from
        //    m->iou, the amount is less than 1 drop (after rounding up the float
        //    representation).
        ...
        for (auto withFix : {false, true})
        {
            ...
            env.fund (M (10000), alice, bob, carol, dan, erin, gw);
    ...
    void testEnforceNoMTChain ()
    {
        testcase ("Enforce No MTChain");
        ...
        {
            // No mtchain with an implied account step after an offer
            Env env {*this};
        ...
        }
        {
            ...
            env.fund (M (10000), alice, bob, carol, dan, gw1, gw2);
    ...
    testInsufficientReserve ()
    {
        ...
        auto const mOffer = M (1000);

        // No crossing:
        {
            Env env {*this};
            env.fund (M (1000000), gw);
            ...
            env (offer (alice, mOffer, usdOffer), ter(tecINSUF_RESERVE_OFFER));
        ...
        // Partial cross:
        {
            ...
            auto const mOffer2 = M (500);

            env.fund (r + f + mOffer, bob);
            env (offer (bob, usdOffer2, mOffer2),   ter(tesSUCCESS));
            env.fund (r + f, alice);
            env (trust (alice, usdOffer),             ter(tesSUCCESS));
            env (pay (gw, alice, usdOffer),           ter(tesSUCCESS));
            env (offer (alice, mOffer, usdOffer),   ter(tesSUCCESS));

            env.require (
                balance (alice, r - f + mOffer2),
                balance (alice, usdOffer2),
                owners (alice, 1),
                balance (bob, r + mOffer2),
        ...
        // buy M. If it fully crosses, we succeed.
        {
            Env env {*this};
            env.fund (M (1000000), gw);
            ...
            env.fund (r + f + mOffer, bob, carol);
            env (offer (bob, usdOffer2, mOffer2),    ter(tesSUCCESS));
            env (offer (carol, usdOffer, mOffer),    ter(tesSUCCESS));
            ...
            env.require (
                balance (alice, r - f + mOffer),
                balance (alice, USD(0)),
                owners (alice, 1),
                balance (bob, r + mOffer2),
                balance (bob, usdOffer2),
                owners (bob, 1),
                balance (carol, r + mOffer2),
    ...
    testFillModes ()
    {
        ...
        auto const startBalance = M (1000000);
        auto const gw = Account {"gateway"};
        auto const alice = Account {"alice"};
        auto const bob = Account {"bob"};
        auto const USD = gw["USD"];
        ...
        // a fee and not place the offer on the books:
        {
            ...
            env (offer (alice, M (500), USD (500)),
                txflags (tfFillOrKill),              ter(tesSUCCESS));

            env.require (
                balance (alice, startBalance - f - f - f + M (500)),
                balance (alice, USD (500)),
                owners (alice, 1),
                offers (alice, 0),
                balance (bob, startBalance - f - M (500)),
                balance (bob, USD (500)),
                owners (bob, 1),
                offers (bob, 0));
        }
        ...
        // and add nothing on the books:
        {
            ...
            // Partially cross:
            env (offer (bob, USD (50), M (50)),            ter(tesSUCCESS));
            env (offer (alice, M (1000), USD (1000)),
                txflags (tfImmediateOrCancel),               ter(tesSUCCESS));

            env.require (
                balance (alice, startBalance - f - f - f + M (50)),
                balance (alice, USD (950)),
                owners (alice, 1),
                offers (alice, 0),
                balance (bob, startBalance - f - M (50)),
                balance (bob, USD (50)),
                owners (bob, 1),
                offers (bob, 0));
            ...
            // Fully cross:
            env (offer (bob, USD (50), M (50)),            ter(tesSUCCESS));
            env (offer (alice, M (50), USD (50)),
                txflags (tfImmediateOrCancel),               ter(tesSUCCESS));

            env.require (
                balance (alice, startBalance - f - f - f - f + M (100)),
                balance (alice, USD (900)),
                owners (alice, 1),
                offers (alice, 0),
                balance (bob, startBalance - f - f - M (100)),
                balance (bob, USD (100)),
                owners (bob, 1),
                offers (bob, 0));
        }
    ...
    testMalformed()
    {
        ...
        {
            // Alice tries an M to M order:
            env (offer (alice, M (1000), M (1000)),   ter(temBAD_OFFER));
            env.require (
                owners (alice, 0),
                offers (alice, 0));
        ...
        {
            env (offer (alice, -USD (1000), M (1000)),  ter(temBAD_OFFER));
            env.require (
                owners (alice, 1),
                offers (alice, 0));
        ...
        // Use M as a currency code
        {
            auto const BAD = IOU(gw, badCurrency());

            env (offer (alice, M (1000), BAD (1000)),   ter(temBAD_CURRENCY));
            env.require (
                owners (alice, 1),
                offers (alice, 0));
        }
    ...
    testExpiration()
    {
        ...
        env (offer (alice, mOffer, usdOffer),
        ...
        env (offer (bob, usdOffer, mOffer),     ter(tesSUCCESS));
        env.require (
            balance (alice, startBalance - f - f - f),
            balance (alice, usdOffer),
            offers (alice, 0),
    ...
    testUnfundedCross()
    {
        ...
        env (offer ("alice", usdOffer, mOffer),     ter(tecUNFUNDED_OFFER));
        env.require (
        ...
        env (offer ("bob", usdOffer, mOffer),       ter(tecUNFUNDED_OFFER));
        env.require (
        ...
        env.fund (reserve (env, 0) + f + M(1), "carol");
        env (offer ("carol", usdOffer, mOffer),     ter(tecINSUF_RESERVE_OFFER));
        env.require (
            balance ("carol", reserve (env, 0) + M(1)),
            owners ("carol", 0));

        // Account has enough for the reserve plus one
        // offer, and the fee.
        env.fund (reserve (env, 1) + f, "dan");
        env (offer ("dan", usdOffer, mOffer),       ter(tesSUCCESS));
        env.require (
        ...
        env.fund (reserve (env, 1) + f + mOffer, "eve");
        env (offer ("eve", usdOffer, mOffer),       ter(tesSUCCESS));
        env.require (
            balance ("eve", reserve (env, 1) + mOffer),
            owners ("eve", 1));
    }
    ...
    testSelfCross(bool use_partner)
    {
        ...
        // through M
        env (offer (account_to_test, BTC (250), M (1000)),
                 offers (account_to_test, 1));

        // validate that the book now shows a BTC for M offer
        BEAST_EXPECT(isOffer(env, account_to_test, BTC(250), M(1000)));

        auto const secondLegSeq = env.seq(account_to_test);
        env (offer (account_to_test, M(1000), USD (50)),
                 offers (account_to_test, 2));

        // validate that the book also shows a M for USD offer
        BEAST_EXPECT(isOffer(env, account_to_test, M(1000), USD(50)));
        ...
        jrr = getBookOffers(env, BTC, M);
        BEAST_EXPECT(jrr[jss::offers].isArray());
        BEAST_EXPECT(jrr[jss::offers].size() == 0);

        BEAST_EXPECT(isOffer(env, account_to_test, M(0), USD(0)));
    ...
    testNegativeBalance()
    {
        ...
        env (offer (alice, USD (50), M (150000)));
    ...
    void
    testOfferCrossWithM(bool reverse_order)
    {
        testcase (std::string("Offer Crossing with M, ") +
        ...
        if (reverse_order)
            env (offer (bob, USD (1), M (4000)));

        env (offer (alice, M (150000), USD (50)));
        ...
        BEAST_EXPECT(
            jrr[jss::node][sfBalance.fieldName] ==
            std::to_string(
                M (10000).value ().mantissa () -
                M (reverse_order ? 4000 : 3000).value ().mantissa () -
    ...
    testOfferCrossWithLimitOverride()
    {
        ...
        env.fund (M (100000), gw, alice, bob);
        ...
        BEAST_EXPECT(
            jrr[jss::node][sfBalance.fieldName] ==
            std::to_string(
                M (100000).value ().mantissa () -
                M (3000).value ().mantissa () -
    ...
    testOfferAcceptThenCancel()
    {
        ...
        env (offer (env.master, M (500), USD (100)));
        env.close ();
    ...
    testCurrencyConversionEntire()
    {
        ...
        BEAST_EXPECT(
            jro[jss::node][jss::TakerGets] == M (500).value ().getText ());
        BEAST_EXPECT(
            jro[jss::node][jss::TakerPays] == USD (100).value ().getJson (0));

        env (pay (alice, alice, M (500)), sendmax (USD (100)));
    ...
    testCurrencyConversionInParts()
    {
        ...
        env (pay (alice, alice, M (200)), sendmax (USD (100)));

        // The previous payment reduced the remaining offer amount by 200 M
        auto jro = ledgerEntryOffer (env, bob, bobOfferSeq);
        BEAST_EXPECT(
            jro[jss::node][jss::TakerGets] == M (300).value ().getText ());
        BEAST_EXPECT(
        ...
        // alice now has 200 more M from the payment
        jrr = ledgerEntryRoot (env, alice);
        BEAST_EXPECT(
            jrr[jss::node][sfBalance.fieldName] ==
            std::to_string(
                M (10000).value ().mantissa () +
                M (200).value ().mantissa () -
                env.current ()->fees ().base * 2)
        );
        ...
        // Alice converts USD to M which should fail
        // due to PartialPayment.
        env (pay (alice, alice, M (600)), sendmax (USD (100)),
            ter(tecPATH_PARTIAL));

        // Alice converts USD to M, should succeed because
        ...
        // only 300 M should be have been payed since that's all
    ...
    void
    testCrossCurrencyStartM()
    {
        testcase ("Cross Currency Payment: Start with M");
        ...
        env (offer (carol, M (500), USD (50)));

        env (pay (alice, bob, USD (25)), sendmax (M (333)));
        ...
        BEAST_EXPECT(
            jro[jss::node][jss::TakerPays] == M (250).value ().getText ());
    }

    void
    testCrossCurrencyEndM()
    {
        testcase ("Cross Currency Payment: End with M");
        ...
        auto const carolOfferSeq = env.seq (carol);
        env (offer (carol, USD (50), M (500)));

        env (pay (alice, bob, M (250)), sendmax (USD (333)));
        ...
        BEAST_EXPECT(
            ...
            std::to_string(
                M (10000).value ().mantissa () +
                M (250).value ().mantissa ())
        );

        auto jro = ledgerEntryOffer (env, carol, carolOfferSeq);
        BEAST_EXPECT(
            jro[jss::node][jss::TakerGets] == M (250).value ().getText ());
        BEAST_EXPECT(
    ...
    testCrossCurrencyBridged()
    {
        ...
        env.fund (M (10000), gw1, gw2, alice, bob, carol, dan);
        ...
        env (offer (dan, M (500), EUR (50)));

        Json::Value jtp {Json::arrayValue};
        jtp[0u][0u][jss::currency] = "M";
        ...
        auto jro = ledgerEntryOffer (env, carol, carolOfferSeq);
        BEAST_EXPECT(
            jro[jss::node][jss::TakerGets] == M (200).value ().getText ());
        BEAST_EXPECT(
        ...
        BEAST_EXPECT(
            jro[jss::node][jss::TakerPays] == M (200).value ().getText ());
    }
    ...
    testOfferFeesConsumeFunds()
    {
        ...
        //  1 for payment          == 4
        auto const starting_m = M (100) +
            env.current ()->fees ().accountReserve (3) +
            env.current ()->fees ().base * 4;

        env.fund (starting_m, gw1, gw2, gw3, alice, bob);
        ...
        env (offer (bob, M (200), USD1 (200)));
        // Alice has 350 fees - a reserve of 50 = 250 reserve = 100 available.
        // Ask for more than available to prove reserve works.
        env (offer (alice, USD1 (200), M (200)));

        auto jrr = ledgerEntryState (env, alice, gw1, "USD");
        BEAST_EXPECT(jrr[jss::node][sfBalance.fieldName][jss::value] == "100");
        jrr = ledgerEntryRoot (env, alice);
        BEAST_EXPECT(
            jrr[jss::node][sfBalance.fieldName] == M (350).value ().getText ()
        );
    ...
    testSellFlagBasic()
    {
        ...
        env.fund (starting_m, gw, alice, bob);
        ...
        env (offer (bob, M (200), USD (200)), json(jss::Flags, tfSell));
        // Alice has 350 + fees - a reserve of 50 = 250 reserve = 100 available.
        // Alice has 350 + fees - a reserve of 50 = 250 reserve = 100 available.
        // Ask for more than available to prove reserve works.
        env (offer (alice, USD (200), M (200)), json(jss::Flags, tfSell));

        auto jrr = ledgerEntryState (env, alice, gw, "USD");
        BEAST_EXPECT(jrr[jss::node][sfBalance.fieldName][jss::value] == "-100");
        jrr = ledgerEntryRoot (env, alice);
        BEAST_EXPECT(
            jrr[jss::node][sfBalance.fieldName] == M (250).value ().getText ()
        );
    ...
    testSellFlagExceedLimit()
    {
        ...
        env (offer (bob, M (100), USD (200)));
        // Alice has 350 fees - a reserve of 50 = 250 reserve = 100 available.
        // Ask for more than available to prove reserve works.
        // Taker pays 100 USD for 100 M.
        // Selling M.
        // Will sell all 100 M and get more USD than asked for.
        env (offer (alice, USD (100), M (100)), json(jss::Flags, tfSell));
    ...
    testGatewayCrossCurrency()
    {
        ...
        if (wsc->version() == 2)
        {
            ...
            BEAST_EXPECT(jrr.isMember(jss::FinPalrpc) && jrr[jss::FinPalrpc] == "2.0");
    ...
    void run ()
    {
        ...
        testSelfCross (true);
        testNegativeBalance ();
        testOfferCrossWithM (true);
        testOfferCrossWithM (false);
        testOfferCrossWithLimitOverride ();
        testOfferAcceptThenCancel ();
        testOfferCancelPastAndFuture ();
        testCurrencyConversionEntire ();
...
BEAST_DEFINE_TESTSUITE (Offer, tx, mtchain);
