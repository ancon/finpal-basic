...
class Discrepancy_test : public beast::unit_test::suite
{
    ...
    testMDiscrepancy ()
    {
        testcase ("Discrepancy test : M Discrepancy");
        ...
        env.fund(M(2000), A1);
        env.fund(M(1000), A2, A6, A7);
        env.fund(M(5000), A3);
        env.fund(M(1000000), A4);
        env.fund(M(600000), A5);
        env.close();
        ...
        env(offer(A4, M(49147), A2["JPY"](34501)));
        env(offer(A5, A3["CNY"](3150), M(80086)));
        env(offer(A7, M(1233), A6["CNY"](25)));
        env.close();

        test::PathSet payPaths {
            test::Path {A2["JPY"], A2},
            test::Path {M, A2["JPY"], A2},
            test::Path {A6, M, A2["JPY"], A2} };
...
BEAST_DEFINE_TESTSUITE (Discrepancy, app, mtchain);
