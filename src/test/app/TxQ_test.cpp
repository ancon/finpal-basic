...
public:
    void testQueue()
    {
        ...
        env.fund(M(50000), nomtchain(alice, bob, charlie, daria));
        ...
        env.fund(M(50000), nomtchain(elmo, fred, gwen, hank));
    ...
    void testLocalTxRetry()
    {
        ...
        // Create several accounts while the fee is cheap so they all apply.
        env.fund(M(50000), nomtchain(alice, bob, charlie));
    ...
    void testLastLedgerSeq()
    {
        ...
        env.fund(M(1000), nomtchain(alice, bob));
        env.close(env.now() + 5s, 10000ms);
        env.fund(M(1000), nomtchain(charlie, daria));
        env.close(env.now() + 5s, 10000ms);
        env.fund(M(1000), nomtchain(edgar, felicia));
        env.close(env.now() + 5s, 10000ms);
    ...
    void testZeroFeeTxn()
    {
        ...
        env.fund(M(1000), nomtchain(alice, bob));
        env.close(env.now() + 5s, 10000ms);
        env.fund(M(1000), nomtchain(carol));
        env.close(env.now() + 5s, 10000ms);
    ...
    void testPreclaimFailures()
    {
        ...
        env.fund(M(1000), nomtchain(alice));
        ...
        // Fail in preflight
        env(pay(alice, bob, M(-1000)),
            ter(temBAD_AMOUNT));

        // Fail in preclaim
        env(noop(alice), fee(M(100000)),
            ter(terINSUF_FEE_B));
    }
    ...
    void testMultiTxnPerAccount()
    {
        ...
        // Create several accounts while the fee is cheap so they all apply.
        env.fund(drops(2000), nomtchain(alice));
        env.fund(M(500000), nomtchain(bob, charlie, daria));
        checkMetrics(env, 0, boost::none, 4, 3, 256);
        ...
        aliceFee = env.le(alice)->getFieldAmount(sfBalance).m().drops()
            - (59);
        env(noop(alice), seq(aliceSeq),
            fee(aliceFee), ter(telCAN_NOT_QUEUE));
        checkMetrics(env, 4, 10, 6, 5, 256);
    ...
    void testDisabled()
    {
        ...
        env.fund(M(50000), nomtchain(alice));
    ...
    void testUnexpectedBalanceChange()
    {
        ...
        env.fund(drops(5000), nomtchain(alice));
        env.fund(M(50000), nomtchain(bob));
        ...
        /* At this point, Alice's transaction is indefinitely
            ...
            will get more M, and it'll process.
        */
    ...
    void testBlockers()
    {
        ...
        env.fund(M(50000), nomtchain(alice, bob));
        env.memoize(charlie);
        env.memoize(daria);
        checkMetrics(env, 0, boost::none, 2, 3, 256);
    ...
    void testInFlightBalance()
    {
        ...
        env.fund(M(50000), nomtchain(alice, charlie), gw);
        checkMetrics(env, 0, boost::none, 4, 3, 256);
        ...
        //////////////////////////////////////////
        // Offer with high M out blocks later txs
        auto aliceSeq = env.seq(alice);
        auto aliceBal = env.balance(alice);

        env.require(balance(alice, M(50000)),
            owners(alice, 0));

        // If this offer crosses, all of alice's
        // M will be taken (except the reserve).
        env(offer(alice, BUX(5000), M(50000)),
            queued);
        ...
        // has plenty of M, because the offer didn't
        // cross (of course).
        env.require(balance(alice, aliceBal - drops(20)),
            owners(alice, 1));

        //////////////////////////////////////////
        // Offer with low M out allows later txs
        fillQueue(env, alice);
        checkMetrics(env, 0, 8, 5, 4, 256);
        aliceSeq = env.seq(alice);
        ...
        // of alice's M will be taken.
        env(offer(alice, BUX(50), M(500)),
            queued);
        ...
        //////////////////////////////////////////
        // Large M payment blocks later txs
        fillQueue(env, alice);
        checkMetrics(env, 0, 10, 6, 5, 256);
        ...
        //////////////////////////////////////////
        // Small M payment allows later txs
        fillQueue(env, alice);
        checkMetrics(env, 0, 12, 7, 6, 256);
        ...
        env.require(balance(alice, aliceBal - M(500) - drops(20)),
            owners(alice, 2));
        ...
        // alice's M balance (other than the fee, of course).
        env(noop(alice), seq(aliceSeq + 1), queued);
        ...
        // M balance, but her USD balance went to charlie.
        env.require(balance(alice, aliceBal - drops(20)),
            balance(alice, USD(0)),
        ...
        //////////////////////////////////////////
        // Large M to IOU payment blocks later txs.

        env(offer(gw, M(500000), USD(50000)));
        // Close so we don't have to deal
        // with tx ordering in consensus.
        env.close();
        ...
        // entire M balance to charlie in the
        // form of USD.
        BEAST_EXPECT(M(60000) > aliceBal);
        env(pay(alice, charlie, USD(1000)),
            sendmax(M(60000)), queued);
        ...
        // to charlie using only a portion of her M balance
        env.require(balance(alice, aliceBal - M(10000) - drops(20)),
            balance(alice, USD(0)),
        ...
        //////////////////////////////////////////
        // Small M to IOU payment allows later txs.
        ...
        // a portion of her M balance to charlie
        // in the form of USD.
        BEAST_EXPECT(aliceBal > M(6001));
        env(pay(alice, charlie, USD(500)),
            sendmax(M(6000)), queued);
        ...
        // So once we close the ledger, alice sent a payment
        // to charlie using only a portion of her M balance
        env.require(balance(alice, aliceBal - M(5000) - drops(20)),
            balance(alice, USD(0)),
    ...
    void testConsequences()
    {
        ...
        env.memoize("carol");
        {
            ...
            BEAST_EXPECT(conseq.potentialSpend == M(0));
        }
    ...
    void testExpirationReplacement()
    {
        ...
        */
        ...
        env.fund(M(500000), nomtchain(alice, bob));
        checkMetrics(env, 0, boost::none, 2, 1, 256);
    ...
    void testServerSubscribe()
    {
        ...
        env.fund(M(50000), nomtchain(a,b,c,d));
        checkMetrics(env, 0, boost::none, 4, 3, 256);
        ...
        env.fund(M(50000), nomtchain(e,f,g,h,i));
    ...
    void testClearQueuedAccountTxs()
    {
        ...
        testcase("clear queue failure (load)");
        {
            ...
            env(noop(alice), fee(M(1)), seq(aliceSeq++), ter(terQUEUED));
...
BEAST_DEFINE_TESTSUITE(TxQ,app,mtchain);
