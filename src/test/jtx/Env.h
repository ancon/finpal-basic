...
#ifndef MTCHAIN_TEST_JTX_ENV_H_INCLUDED
#define MTCHAIN_TEST_JTX_ENV_H_INCLUDED
...
/** Designate accounts as no-mtchain in Env::fund */
template <class... Args>
std::array<Account, 1 + sizeof...(Args)>
nomtchain (Account const& account, Args const&... args)
{
    return {{account, args...}};
}
...
public:
    ...
    /** Returns the current MTChain Network Time
    ...
    */
    ...
    /** Returns the M balance on an account.
...
public:
    ...
    /** Create a new account with some M.
        ...
        Effects:
            The asfDefaultMtchain on the account is set,
            and the sequence number is incremented, unless
            the account is wrapped with a call to nomtchain.

            The account's M balance is set to amount.

            Generates a test that the balance is set.

        @param amount The amount of M to transfer to
                      each account.

        @param args A heterogeneous list of accounts to fund
                    or calls to nomtchain with lists of accounts
                    to fund.
    */
    template<class Arg, class... Args>
    void
