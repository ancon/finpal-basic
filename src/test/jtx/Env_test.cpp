...
public:
    ...
    testAmount()
    {
        ...
        BEAST_EXPECT(to_string(M(5)) == "5 M");
        BEAST_EXPECT(to_string(M(.80)) == "0.8 M");
        BEAST_EXPECT(to_string(M(.005)) == "5000 drops");
        BEAST_EXPECT(to_string(M(0.1)) == "0.1 M");
        BEAST_EXPECT(to_string(M(10000)) == "10000 M");
        BEAST_EXPECT(to_string(drops(10)) == "10 drops");
        BEAST_EXPECT(to_string(drops(123400000)) == "123.4 M");
        BEAST_EXPECT(to_string(M(-5)) == "-5 M");
        BEAST_EXPECT(to_string(M(-.99)) == "-0.99 M");
        BEAST_EXPECT(to_string(M(-.005)) == "-5000 drops");
        BEAST_EXPECT(to_string(M(-0.1)) == "-0.1 M");
        BEAST_EXPECT(to_string(drops(-10)) == "-10 drops");
        BEAST_EXPECT(to_string(drops(-123400000)) == "-123.4 M");

        BEAST_EXPECT(M(1) == drops(1000000));
        BEAST_EXPECT(M(1) == STAmount(1000000));
        BEAST_EXPECT(STAmount(1000000) == M(1));
    ...
    testEnv()
    {
        ...
        // fund
        {
            ...
            env.fund(n, "dave", nomtchain("eric"));
            env.fund(n, "fred", nomtchain("gary", "hank"));
            env.fund(n, nomtchain("irene"));
            env.fund(n, nomtchain("jim"), "karen");
            env.fund(n, nomtchain("lisa", "mary"));

            // flags
            env.fund(n, nomtchain("xavier"));
            env.require(nflags("xavier", asfDefaultMtchain));
            env.fund(n, "yana");
            env.require(flags("yana", asfDefaultMtchain));
        }
    ...
    testRequire()
    {
        ...
        env.require(balance("alice", M(none)));
        env.fund(M(10000), "alice", gw);
        env.require(balance("alice", USD(none)));
        env.trust(USD(100), "alice");
        env.require(balance("alice", M(10000))); // fee refunded
    ...
    testPayments()
    {
        ...
        env(offer("carol", M(50), USD(50)), require(owners("carol", 2)));
        env(pay("alice", "bob", any(USD(10))),                  ter(tecPATH_DRY));
        env(pay("alice", "bob", any(USD(10))),
            paths(M), sendmax(M(10)),                       ter(tecPATH_PARTIAL));
        env(pay("alice", "bob", any(USD(10))), paths(M),
            sendmax(M(20)));
...
BEAST_DEFINE_TESTSUITE(Env,app,mtchain);
