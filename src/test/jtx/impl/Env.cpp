...
void
Env::fund (bool setDefaultMTChain,
    STAmount const& amount,
        Account const& account)
{
    memoize(account);
    if (setDefaultMTChain)
    {
        ...
        apply(pay(master, account, amount +
            drops(current()->fees().base)),
                jtx::seq(jtx::autofill),
                    fee(jtx::autofill),
                        sig(jtx::autofill));
        apply(fset(account, asfDefaultMtchain),
            jtx::seq(jtx::autofill),
                fee(jtx::autofill),
                    sig(jtx::autofill));
        require(flags(account, asfDefaultMtchain));
    }
    else
    {
        apply(pay(master, account, amount),
            jtx::seq(jtx::autofill),
                fee(jtx::autofill),
                    sig(jtx::autofill));
        require(nflags(account, asfDefaultMtchain));
    }
...
Env::do_rpc(std::vector<std::string> const& args)
{
    ...
    if (jv.isMember(jss::jsonrpc))
    {
        ...
        response[jss::FinPalrpc] = jv[jss::FinPalrpc];
