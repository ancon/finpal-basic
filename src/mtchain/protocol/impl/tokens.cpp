...
#define ACCOUNT_PREFIX "FPA"
static auto const ACCOUNT_PREFIX_LEN = sizeof(ACCOUNT_PREFIX) - 1;

namespace mtchain {

static char mtchainAlphabet[] =
    "Fpshnaf39wBUDNEGHJKLM4PQRST7VWXYZ2bcdeCg65jkm8orqi1tuvAxyz";
    ...
    void const* token, std::size_t size)
{
    ...
    auto s = encodeBase58(temp, expanded,
        temp + expanded, mtchainAlphabet);

    if (type == TOKEN_ACCOUNT_ID)
    {
        return ACCOUNT_PREFIX + s.substr(1);
    }
    ...
    int type, InverseArray const& inv)
{
    ...
    if (type == TOKEN_ACCOUNT_ID)
    {
        if (s.substr(0, ACCOUNT_PREFIX_LEN) != ACCOUNT_PREFIX)
            return result;

        result = decodeBase58(std::string(1, mtchainAlphabet[0]) + s.substr(ACCOUNT_PREFIX_LEN), inv);
    }
    else
    {
        result = decodeBase58(s, inv);
    }
...
static InverseAlphabet mtchainInverse(mtchainAlphabet);
