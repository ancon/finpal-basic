...
STAmount::STAmount (MAmount const& amount)
    : mOffset (0)
    , mIsNative (true)
    , mIsNegative (amount < zero)
{
...
//
//------------------------------------------------------------------------------
MAmount STAmount::m () const
{
    if (!mIsNative)
        Throw<std::logic_error> ("Cannot return non-native STAmount as MAmount");
...
void STAmount::setJson (Json::Value& elem, int options) const
{
...
Json::Value
STAmount::getJson (int options) const
{
    Json::Value elem;
    setJson (elem, options);
    return elem;
}
...
amountFromString (Issue const& issue, std::string const& amount)
{
    ...
    // Can't specify M using fractional representation
    if (isM(issue) && match[3].matched)
        Throw<std::runtime_error> ("M must be specified in integral drops.");
...
amountFromJson (SField const& name, Json::Value const& v)
{
    ...
    {
        if (v.isObject ())
            Throw<std::runtime_error> ("M may not be specified as an object");
        issue = mIssue ();
    }
    else
    {
...
multiply (STAmount const& v1, STAmount const& v2, Issue const& issue)
{
    ...
    if (v1.native() && v2.native() && isM (issue))
    {
    ...
    bool roundUp)
{
    ...
    bool const m = isM (issue);

    if (v1.native() && v2.native() && m)
    {
    ...
    if (resultNegative != roundUp)
        canonicalizeRound (m, amount, offset);
    ...
    Issue const& issue, bool roundUp)
{
    ...
    if (resultNegative != roundUp)
        canonicalizeRound (isM (issue), amount, offset);

    STAmount result (issue, amount, offset, resultNegative);
    // Control when bugfixes that require switchover dates are enabled
    if (roundUp && !resultNegative && !result && *stAmountCalcSwitchover)
    {
        if (isM(issue) && *stAmountCalcSwitchover2)
        {
            // return the smallest value above zero
            amount = 1;
            offset = 0;
...
STAmount multiply(std::tuple<std::uint64_t, std::uint64_t> const& fraction,
		  std::uint64_t factor, Issue const& issue)
{
    std::uint64_t num, den;
    std::tie(num, den) = fraction;

    STAmount amount(issue);
    if (den)
    {
        amount = multiply(STAmount((factor + den - 1) / den), STAmount(num), issue);
    }
