...
template <>
struct hash <mtchain::error_code_i>
{
    std::size_t operator() (mtchain::error_code_i value) const
    {
        return value;
    }
};
...
public:
        ...
        : m_unknown (rpcUNKNOWN, "unknown", "An unknown error code.")
    {
        ...
        add (rpcNO_NETWORK,            "noNetwork",         "Not synced to FinPal network.");
        add (rpcNO_PATH,               "noPath",            "Unable to find a FinPal path.");
        ...
        add (rpcNO_ASSET,              "noAsset",           "No such asset.");
        add (rpcNO_TOKEN,              "noToken",           "No such token.");
        add (rpcNO_OWNER,              "noOwner",           "No such owner account.");
    }
