...
bool STObject::setType (const SOTemplate& type)
{
    ...
    decltype(v_) v;
    v.reserve(type.size());
    std::string err_msg_all = "";
    std::string err_msg_explicit = "";
    std::string err_msg_missing = "";
    std::string err_msg_extra = "";
    bool isFirstExplicit = false;
    bool isFirstMissing = false;
    bool isFirstExtra = false;
    for (auto const& e : type.all())
    {
		...
		if(!isFirstExplicit)
		{
			err_msg_explicit += "Explicit field: 'tx_json."+e->e_field.fieldName+"'";
			isFirstExplicit = true;
		}else{
			err_msg_explicit += ",'tx_json."+ e->e_field.fieldName + "'";
		}
            }
		...
		if(!isFirstMissing)
		{
			err_msg_missing += "Missing field: 'tx_json."+e->e_field.fieldName+"' ";
			isFirstMissing = true;
		}else{
			err_msg_missing += ",'tx_json."+ e->e_field.fieldName +"' ";
		}
            }
            v.emplace_back(detail::nonPresentObject, e->e_field);
        }
    }
    if(isFirstExplicit) err_msg_all += err_msg_explicit+ "; ";
    if(isFirstMissing) err_msg_all += err_msg_missing + "; ";
    for (auto const& e : v_)
    {
	    ...
	    if(!isFirstExtra)
	    {
		err_msg_extra += "Extra field: 'tx_json."+e->getFName().getName()+"' ";
	    }else{
	    	err_msg_extra += ",'tx_json."+ e->getFName().getName () + "' ";
	    }
	    isFirstExtra = true;
        }
    }
	
    err_msg_all += err_msg_extra;
    if(err_msg_all != "")
    {
	Throw<std::invalid_argument>(err_msg_all);
    }
...
void STObject::add (Serializer& s, bool withSigningFields) const
{
    ...
    for (auto const& e : fields)
    {
        ...
        const STObject* obj = dynamic_cast<const STObject*> (field);
        if (obj != nullptr)
        {
            obj->add (s, withSigningFields);
            s.addFieldID (STI_OBJECT, 1);
        }
        else
        {
            const STArray* arr = dynamic_cast<const STArray*> (field);
            if (arr != nullptr)
            {
                arr->add (s, withSigningFields);
...
bool STObject::append (STObject const& obj)
{
    if (!isFree())
    {
        return false;
    ...
    {
         index = getFieldIndex (e.getFName());
         if (index != -1) continue;
