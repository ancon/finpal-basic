...
/*
    ...
    Answer (David Schwartz):
        ...
        The longer answer was that:
            1) Using a single hash could leave mtchain
...
genesisAccount()
{
    static auto const pioneer = calcAccountID(
        generateKeyPair(KeyType::secp256k1,
            generateSeed("NothingIsImpossible")).first);
    return pioneer;
}
...
firstAccount()
{
    static auto const first = calcAccountID(generateKeyPair(KeyType::secp256k1,
        generateSeed("XiangLongShiBaZhang")).first);
...
{
    static auto const first = calcAccountID(generateKeyPair(KeyType::secp256k1,
        generateSeed("DuGuJiuJian")).first);
