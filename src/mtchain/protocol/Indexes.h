...
#ifndef MTCHAIN_PROTOCOL_INDEXES_H_INCLUDED
#define MTCHAIN_PROTOCOL_INDEXES_H_INCLUDED
...
struct nfasset_t
{
    Keylet operator()(AccountID const& issuer, Blob const& id) const;
    Keylet operator()(uint256 const& assetid, AccountID const& owner) const;
    Keylet operator()(uint256 const& assetid, AccountID const& owner, AccountID const& oper) const;

    Keylet operator()(uint256 const& key) const
    {
        return { ltNFASSET, key };
    }

    Keylet operator()(AccountID const& account, std::uint64_t assetIndex) const;
};
static nfasset_t const nfasset {};

struct nftoken_t
{
    Keylet operator()(uint256 const& assetid, Blob const& id) const;
    Keylet operator()(uint256 const& assetid, std::uint64_t tokenIndex) const;
    Keylet operator()(uint256 const& assetid, AccountID const& owner,
                      std::uint64_t ownerTokenIndex) const;

    Keylet operator()(uint256 const& key) const
    {
        return { ltNFTOKEN,  key };
    }
};
static nftoken_t const nftoken {};
