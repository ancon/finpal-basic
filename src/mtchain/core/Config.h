...
#ifndef MTCHAIN_CORE_CONFIG_H_INCLUDED
#define MTCHAIN_CORE_CONFIG_H_INCLUDED

#include <mtchain/basics/BasicConfig.h>
#include <mtchain/basics/base_uint.h>
#include <mtchain/protocol/SystemParameters.h> // VFALCO Breaks levelization
...
public:
    ...
    static std::string const configFileName;
...
public:
    ...
    std::vector<std::string>    IPS;                    // Peer IPs from MTChain-core.cfg.
    std::vector<std::string>    IPS_FIXED;              // Fixed Peer IPs from MTChain-core.cfg.
    std::vector<std::string>    SNTP_SERVERS;           // SNTP servers from MTChain-core.cfg.
    std::vector<std::pair<AccountID, std::uint32_t> > MINERS;
    ...
    // Network parameters
    int const                   TRANSACTION_FEE_BASE = 100;   // The number of fee units a reference transaction costs
    ...
    std::uint64_t                      FEE_DEFAULT = 100;
    std::uint64_t                      FEE_ACCOUNT_RESERVE = 0.0001*SYSTEM_CURRENCY_PARTS;
    std::uint64_t                      FEE_OWNER_RESERVE = 0.0005*SYSTEM_CURRENCY_PARTS;
