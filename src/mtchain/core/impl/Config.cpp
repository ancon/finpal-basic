...
std::string const Config::configFileName = systemName() + "-core.cfg";
...
void Config::loadFromString (std::string const& fileContents)
{
    ...
    if (auto s = getIniFileSection (secConfig, SECTION_MINERS))
    {
        std::string id, ratio;
        for (auto const& m : *s)
        {
            auto i = m.find_first_of(' ');
            id = m.substr(0, i);

            i = m.find_first_not_of(' ', i);
            ratio = i != std::string::npos ? m.substr(i) : "";

            auto account = parseBase58<AccountID> (id);
            if (!account)
            {
                Throw<std::runtime_error> ("invalid miner account: " + id);
            }

            MINERS.emplace(MINERS.end(), *account,
                !ratio.empty() ? beast::lexicalCastThrow <std::uint32_t> (ratio) : 0U);
        }
    }

    //if (MINERS.size() < 4) 
    //    Throw<std::runtime_error> ("Don't have enough miners");
...
#ifdef IPFS_ENABLE
    getSingleSection (secConfig, SECTION_FILE_UPLOAD_DIR, FILE_UPLOAD_DIR, j_);
#endif
