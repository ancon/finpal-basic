...
#include <mtchain/beast/container/aged_unordered_set.h>
#include <cstdint>
#include <sstream>
#include <stdexcept>
...
// the FinPald server-server use case, where we only need MITM
...
info_handler (SSL const* ssl, int event, int)
{
    ...
    {
        if (disallowRenegotiation (ssl, SSL_in_before ((SSL *)ssl)))
        {
            ...
            #endif
            LogThrow("Should disallow the ssl, but unsupport set SSL3_FLAGS_NO_RENEGOTIATE_CIPHERS!");
        }
    }
}
...
get_context (std::string cipherList)
{
    ...
    // These are the raw DH parameters that FinPal Alliance has
    // chosen for MTChain, in the binary format needed by
    // d2i_DHparams.
    //
