...
#ifndef MTCHAIN_BASICS_RANDOM_H_INCLUDED
#define MTCHAIN_BASICS_RANDOM_H_INCLUDED
...
static_assert (
    ...
    std::is_unsigned <beast::xor_shift_engine::result_type>::value,
        "The MTChain default PRNG engine must return an unsigned integral type.");

static_assert (
    std::numeric_limits<beast::xor_shift_engine::result_type>::max() >=
    std::numeric_limits<std::uint64_t>::max(),
        "The MTChain default PRNG engine return must be at least 64 bits wide.");
#endif
...
inline
void rand(Buffer &buf)
{
    beast::rngfill (buf.begin(), buf.size(), crypto_prng());
}

inline uint256 rand_uint256()
{
    uint256 r;
    rand(r);
    return r;
...
#endif // MTCHAIN_BASICS_RANDOM_H_INCLUDED
