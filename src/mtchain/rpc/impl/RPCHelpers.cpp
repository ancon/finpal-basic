    ...
    std::shared_ptr<Transaction> transaction, TxMeta::pointer transactionMeta)
{
    ...
    {  // 硬性的从某个账本高度开始加入某个处理, 此方法不适合重新开始架新链
        meta[jss::delivered_amount] =
...
injectSLE(Json::Value& jv, SLE const& sle)
{
	...
	if (sle.isFieldPresent(sfTransferFee))
        {
            jv[jss::TransferFee] = sle.getFieldAmount(sfTransferFee).getJson(0);
	}

        jv.removeMember(sfIssues.getName());
    }
    else
    {
        jv[jss::Invalid] = true;
...
keypairForSignature(Json::Value const& params, Json::Value& error)
{
    ...
    if (has_key_type)
    {
        ...
        if (seed)
        {
            ...
            {
                if (seed->type() != keyType)
                {
                    error = RPC::make_error(rpcBAD_SEED, "keypairForSignature: "
    ...
    if (/* keyType != KeyType::secp256k1 && keyType != KeyType::ed25519 && */
        !isSupportKeyType (keyType))
    {
        error = RPC::make_error(rpcINVALID_PARAMS, "keypairForSignature: invalid key type");
