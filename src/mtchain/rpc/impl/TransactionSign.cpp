...
#include <mtchain/app/tx/apply.h>              // Validity::Valid
...
static Json::Value checkPayment(Json::Value& tx_json,
                                ...
                                STAmount& sendMax)
{
    ...
    if (sendMax.native () && amount.native ())
        return RPC::make_error (rpcINVALID_PARAMS, "Cannot build M to M paths.");
    ...
    if (ledger)
    {
        Pathfinder pf(std::make_shared<MTChainLineCache>(ledger),
    ...
    bool doPath)
{
    ...
    auto strTransType = tx_json[jss::TransactionType].asString ();
    boost::optional<AccountID> dstAccountID;
    STAmount amount;

    if (strTransType == "Payment")
    {
	...
	dstAccountID = parseBase58<AccountID>(tx_json[jss::Destination].asString());
	if (! dstAccountID)
	    return RPC::invalid_field_error ("tx_json.Destination");

	if (tx_json.isMember (jss::Payees))
	    return RPC::invalid_field_error ("tx_json.Payees");
    }
    else if (strTransType == "MultiPayment")
    {
	if (tx_json.isMember (jss::Amount) &&
	    ! amountFromJsonNoThrow (amount, tx_json [jss::Amount]))
	    return RPC::invalid_field_error ("tx_json.Amount");
	...
	if (tx_json.isMember (jss::Payees))
        {
            if (!tx_json[jss::Payees].isArray())
                return RPC::invalid_field_error ("tx_json.Payees");

            STAmount saDstAmount;
            Json::Value jsonResult;
            for (auto const& obj : tx_json[jss::Payees])
            {
                auto const& payee = obj[jss::Payee];
                if (!payee.isMember(jss::Destination))
	            return RPC::invalid_field_error ("tx_json.Payees.Payee.Destination");

                dstAccountID = parseBase58<AccountID>(payee[jss::Destination].asString());
                if (!dstAccountID)
                    return RPC::invalid_field_error ("tx_json.Payees.Payee.Destination");

                if (payee.isMember(jss::Amount))
                {
                    if (!amountFromJsonNoThrow(saDstAmount, payee[jss::Amount]))
                        return RPC::invalid_field_error ("tx_json.Payees.Payee.Amount");

                    jsonResult = checkPayment(tx_json, srcAddressID, *dstAccountID, saDstAmount,
                                              role, app, ledger, sendMax);
		}
	        else
		{
		    if (!tx_json.isMember (jss::Amount))
                    ...
                    jsonResult = checkPayment(tx_json, srcAddressID, *dstAccountID, amount, role,
                                              app, ledger, sendMax);
                }

                if (contains_error(jsonResult)) return jsonResult;
            }
        }
        else
        {
		...
		return RPC::missing_field_error ("tx_json.Destination");
            ...
            return checkPayment(tx_json, srcAddressID, *dstAccountID, amount, role, app, ledger,
                                sendMax);
        }
    }
    ...
    std::shared_ptr<OpenView const> const& ledger)
{
    ...
    {
        if (!tx_json.isMember (jss::Sequence)) // set the transaction's seq
        {
            if (! sle)
            {
                JLOG (j.debug())
    ...
    {
	return RPC::make_error(rpcINVALID_PARAMS,std::string(e.what()));
    }
    catch (std::exception& e)
    {
	  return RPC::make_error(rpcINTERNAL,"Exception occurred constructing serialized transaction: " + std::string(e.what()));
    }
...
static Json::Value transactionFormatResultImpl (Transaction::pointer tpTrans)
{
    ...
    try
    {
        ...
        tpTrans->getSTransaction()->insertTo (jvResult);
    }
    catch (std::exception&)
    {
    ...
    std::shared_ptr<OpenView const> const& ledger)
{
    ...
    std::uint64_t const feeDefault = tx.isMember(jss::Payees) && tx[jss::Payees].isArray() &&
        tx[jss::Payees].size() > 1 ? config.TRANSACTION_FEE_BASE * tx[jss::Payees].size() :
        config.TRANSACTION_FEE_BASE;
    ...
    ProcessTransactionFn const& processTransaction)
{
    ...
    // Validate the fields in the serialized transaction.
    {
        ...
        // The Fee field must be in M and greater than zero.
        ...
        if (!isLegalNet (fee))
        {
            ...
            err << "Invalid " << sfFee.fieldName
                << " field.  Fees must be specified in M.";
            return RPC::make_error (rpcINVALID_PARAMS, err.str ());
        }
        if (fee <= 0)
        {
