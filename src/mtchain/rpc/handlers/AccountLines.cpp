...
void addLine (Json::Value& jsonLines, MTChainState const& line)
{
    ...
    if (!line.precision.native())
        jPeer["decimal"] = line.precision.decimal();
    ...
    if (line.getNoMtchain ())
        jPeer[jss::no_FinPal] = true;
    if (line.getNoMtchainPeer ())
        jPeer[jss::no_FinPal_peer] = true;
    if (line.getFreeze ())
...
Json::Value doAccountLines (RPC::Context& context)
{
    ...
    if (params.isMember (jss::marker))
    {
        ...
        auto const sleLine = ledger->read({ltMTCHAIN_STATE, startAfter});
        ...
        auto const line = MTChainState::makeItem (accountID, sleLine);
        if (line == nullptr)
            return rpcError (rpcINVALID_PARAMS);
...

    {
        if (! forEachItemAfter(*ledger, accountID,
                startAfter, startHint, reserve,
                    [&visitData, &ledger](std::shared_ptr<SLE const> const& sleCur)
            {
                auto const line =
                    MTChainState::makeItem (visitData.accountID, sleCur);
                if (line != nullptr &&
                    (! visitData.hasPeer ||
                     visitData.raPeerAccount == line->getAccountIDPeer ()))
                {
                    auto const& issue = line->getLimit().signum() > 0 ? line->getLimitPeer().issue() : line->getLimit().issue();

                    auto k = keylet::line(mAccount(), issue);
                    auto sle = ledger->read(k);
                    if (sle)
                    {
                        line->precision = sle->getFieldAmount(sfHighOut);
                    }  
    ...
    if (visitData.items.size () == reserve)
    {
        ...
        MTChainState::pointer line (visitData.items.back ());
