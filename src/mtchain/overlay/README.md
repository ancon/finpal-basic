...
The _MTChain payment network_ consists of a collection of _peers_ running
**FinPald**. Each peer maintains multiple outgoing connections and optional
...
fully connected directed graph of nodes where vertices are instances of FinPald
...
establishment, defines the _MTChain peer protocol_ (_protocol_ in this context).
...
* _User Defined_ (Unimplemented)
    ...
    The FinPald operator may specify additional, optional fields and values
    through the configuration. These headers will be transmitted in the
    corresponding request or response messages.

# MTChain Clustering #

A cluster consists of more than one MTChain server under common
...
Cluster members are configured in the `MTChain-core.cfg` file under
