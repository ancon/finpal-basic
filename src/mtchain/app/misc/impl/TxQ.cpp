    ...
    TxQ::Setup const& setup)
{
    ...
    // happen.
    return mulDiv(tx[sfFee].m().drops(),
        baseRefLevel,
        ...
        ApplyFlags flags, beast::Journal j)
{
    ...
    if (!allowEscalation)
    {
        return mtchain::apply(app, view, *tx, flags, j);
    }
    ...
    struct MultiTxn
    {
        ...
        MAmount fee = beast::zero;
        MAmount potentialSpend = beast::zero;
        ...
        if (sle)
        {
            ...
            if (multiTxn)
            {
                /* Check if the total fees in flight are greater
                    ...
                    Assume: Minimum account reserve is 20 M.
                    Example 1: If I have 1,000,000 M, I can queue
                        a transaction with a 1,000,000 M fee. In
                        the meantime, some other transaction may
                        lower my balance (eg. taking an offer). When
                        the transaction executes, I will either
                        spend the 1,000,000 M, or the transaction
                        will get stuck in the queue with a
                        `terINSUF_FEE_B`.
                    Example 2: If I have 1,000,000 M, and I queue
                        10 transactions with 0.1 M fee, I have 1 M
                        in flight. I can now queue another tx with a
                        999,999 M fee. When the first 10 execute,
                        ...
                        999,999 M, or get stuck in the queue.
                    Example 3: If I have 1,000,000 M, and I queue
                        7 transactions with 3 M fee, I have 21 M
    ...
    */
    ...
    if (!multiTxn && feeLevelPaid >= requiredFeeLevel)
    {
        ...
        JLOG(j_.trace()) << "Applying transaction " <<
            transactionID <<
            " to open ledger.";
        mtchain::TER txnResult;
        bool didApply;

        std::tie(txnResult, didApply) = doApply(pcresult, app, view);
	if (tx->getTransactionID() != transactionID)
	{
            JLOG(j_.warn()) << "Transaction ID changes from '" << transactionID << "' -> '"
                            << tx->getTransactionID() << "'";

            *const_cast<uint256 *>(&transactionID) = tx->getTransactionID();
	}
