...
MTChaind's fee mechanism consists of several interrelated processes:
...
for the queue if it meets these additional criteria:
  ...
  * all other queued transactions for that account, in the case where
  they spend the maximum possible M, leave enough M balance to pay
  the fee,
  * the total fees for the other queued transactions are less than both
  the network's minimum reserve and the account's M balance, and
...
This demonstrates that a simpler transaction paying less M can be more
likely to get into the open ledger, or be sorted earlier in the queue
than a more complex transaction paying more M.
...
more (or less) work for FinPald to process. Those transactions may have
...
behavior of the mtchain network. Note that this is not necessarily the
...
automatically as the mtchain network's performance improves, allowing
...
not make sense if FinPald is not synced to the network.
...
### [`server_info`](https://mtchain.io/build/FinPald-apis/#server-info) command
...
### [`server_state`](https://mtchain.io/build/FinPald-apis/#server-state) command
