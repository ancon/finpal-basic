...
#ifndef MTCHAIN_APP_MISC_MANIFEST_H_INCLUDED
#define MTCHAIN_APP_MISC_MANIFEST_H_INCLUDED
...
/*
    ...
    Suppose the secret keys installed on a MTChain validator are compromised.  Not
    only do you have to generate and install new key pairs on each validator,
    EVERY FinPald needs to have its config updated with the new public keys, and
    ...
    signed by the master key.  When a FinPald peer receives the new manifest,
