...
#include <mtchain/app/ledger/LocalTxs.h>
...
#include <mtchain/app/misc/impl/AccountTxPaging.h>
...
#include <mtchain/overlay/predicates.h>
...
public:
     ...
     */
    std::shared_ptr<OpenView const> apply (std::unique_lock<std::mutex>& batchLock);
...
private:
    void setHeartbeatTimer (std::chrono::milliseconds delay = LEDGER_GRANULARITY);
...
{
    setHeartbeatTimer (10s);
    setClusterTimer ();
}

void NetworkOPsImp::setHeartbeatTimer (std::chrono::milliseconds delay)
{
    m_heartbeatTimer.setExpiration (delay);
}
        ...
        bool bUnlimited, bool bLocal, FailHard failType)
{
    ...
    {
#ifdef IPFS_ENABLE
        transaction->getSTransaction()->setLocal ();
#endif
        ...
        bool bUnlimited, FailHard failType)
{
    ...
    std::shared_ptr<OpenView const> newOL;
    do
    {
    ...
    lock.unlock();
    afterApply(*newOL, *transaction->getSTransaction(), transaction->getResult(),
               app_, m_journal);
}
...
std::shared_ptr<OpenView const> NetworkOPsImp::apply (std::unique_lock<std::mutex>& batchLock)
{
        ...
        bool changed = false;
        {
                ...
                [&](OpenView& view, beast::Journal j)
            {
                for (TransactionStatus& e : transactions)
                {
                    ...
                    /* 交易Hash正常情况不会改变
                    auto newID = e.transaction->getSTransaction()->getTransactionID();
                    auto oldID = e.transaction->getID();
		    if (newID != oldID)
		    {
                        // must first update transaction ID, else cann't refresh tx cache
                        const_cast<uint256 &>(e.transaction->getID()) = newID;
                        app_.getMasterTransaction().canonicalize(&e.transaction);
                        app_.getMasterTransaction().getCache().del(oldID, false);
		    }
		    */
                }
                return changed;
    ...
    if (sleNode)
    {
        ...
        do
        {
            for (auto const& uDirEntry : sleNode->getFieldV256 (sfIndexes))
            {
                ...
                case ltMTCHAIN_STATE:
                    if (!jvObjects.isMember (jss::FinPal_lines))
                    {
                        jvObjects[jss::FinPal_lines] =
                                Json::Value (Json::arrayValue);
                    }

                    jvObjects[jss::FinPal_lines].append (sleCur->getJson (0));
                    break;
...
void NetworkOPsImp::setMode (OperatingMode om)
{
    if (om == omCONNECTED)
    {
        if (app_.getLedgerMaster ().getValidatedLedgerAge () < 60min)
            om = omSYNCING;
    }
    ...
    if (!human)
    {
        ...
        if (escalationMetrics)
        {
            ...
            /* Json::Value doesn't support uint64, so clamp to max
                ...
                probably isn't enough extant M to drive the factor
                that high.
            */
