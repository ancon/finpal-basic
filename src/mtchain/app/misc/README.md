...
The MTChain payment protocol enforces a fee schedule expressed in units of the
native currency, M. Fees for transactions are paid directly from the account
...
may want to change fees is when the value of the native currency M fluctuates
...
The MTChain consensus algorithm allows distributed participants to arrive at
...
A validating instance of FinPald uses information in the configuration file
...
consensus to succeed, otherwise different instances of FinPald will get 
...
*  A pull request is created and the new code is folded into a FinPald build 
