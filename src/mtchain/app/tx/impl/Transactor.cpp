...
preflight1 (PreflightContext const& ctx)
{
    ...
    if (!fee.native () || fee.negative () || !isLegalAmount (fee.m ()))
    {
    ...
    PreclaimContext const& ctx)
{
    ...
    if (ctx.tx.isFieldPresent(sfPayees))
    {
        auto const& payees = ctx.tx.getFieldArray(sfPayees);
        if (payees.size() > 1) baseFee *= payees.size();
    }
...
Transactor::checkFee (PreclaimContext const& ctx, std::uint64_t baseFee)
{
    ...
    auto const feeDue = mtchain::calculateFee(ctx.app,
    ...
    auto const sle = ctx.view.read(
        keylet::account(id));
    auto const balance = (*sle)[sfBalance].m();

    if (balance < feePaid) // check isf account balance is enough
    {
...
{
    static auto const& id = genesisAccount();
    if (id == account_) return tesSUCCESS;
    ...
    // VFALCO Should we call view().rawDestroyM() here as well?

    // [fee_pool] collect fee to masterpassphrase account	
    auto const sle_m = view().peek(keylet::account(id));

    MAmount masterBalance = STAmount((*sle_m)[sfBalance]).m();
    masterBalance += feePaid;

    sle_m->setFieldAmount(sfBalance, masterBalance);
...
TER Transactor::apply ()
{
    ...
    if (sle)
    {
        mPriorBalance   = STAmount ((*sle)[sfBalance]).m ();
...
{ // verify sign key if belong to the tx account
    auto const id = ctx.tx.getAccountID(sfAccount);
...
Transactor::operator()()
{
    ...
    auto fee = ctx_.tx.getFieldAmount(sfFee).m ();
        ...
        (isTecClaim (terResult) && !(view().flags() & tapRETRY)))
    {
        ...
        auto const balance = txnAcct->getFieldAmount (sfBalance).m ();
...
void addTxFee(STTx const& tx, STAmount const& fee, AccountID const* account)
{
    auto &txFees = tx.meta.peekFieldArray(sfTxFees);
    STObject txFee(sfTxFee);
    txFee.setFieldAmount(sfAmount, fee);
    if (account)
    {
        txFee.setAccountID(sfAccount, *account);
    }
    txFees.push_back(std::move(txFee));
