...
#include <mtchain/conditions/Condition.h>
#include <mtchain/conditions/Fulfillment.h>
...
/*
    ...
    of M in its own ledger entry, until the escrow process
    ...
    sequestered M. If the escrow is, instead, canceled,
    the account which created the escrow will receive the
    sequestered M back instead.
    ...
    EscrowCreate
        ...
        Validation rules:
            ...
            sfCancelAfter
                ...
                specified time (seconds after the MTChain epoch).
...
EscrowCreate::preflight (PreflightContext const& ctx)
{
    ...
    {
        using namespace mtchain::cryptoconditions;
...
EscrowCreate::doApply()
{
    ...
    {
        auto const balance = STAmount((*sle)[sfBalance]).m();
        ...
        if (balance < reserve + STAmount(ctx_.tx[sfAmount]).m())
            return tecUNFUNDED;
    }
