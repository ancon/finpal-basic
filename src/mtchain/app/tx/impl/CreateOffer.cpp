...
CreateOffer::calculateMaxSpend(STTx const& tx)
{
    ...
    return saTakerGets.native() ?
        saTakerGets.m() : beast::zero;
}

bool hasTakerPay(ReadView const& view, AccountID const& owner, STAmount const& in, beast::Journal j)
{
    if (!isM(in))
    {
        auto const& issue = in.issue();
        ...
        {
            assert (total == *limit);
            auto const balance =  accountHolds(view, owner, in.getCurrency(), in.getIssuer(),
                                               fhIGNORE_FREEZE, j);
            if (balance >= total)
            {
...
CreateOffer::preflight (PreflightContext const& ctx)
{
    ...
    {
        JLOG(j.debug()) <<
            "Malformed offer: redundant (M for M)";
        return temBAD_OFFER;
    }
    ...
    // We don't allow a non-native currency to use the currency code M.
...
CreateOffer::preclaim(PreclaimContext const& ctx)
{
        ...
        fhZERO_IF_FROZEN, viewJ) <= zero)
    {
        ...
        return !isM(saTakerGets) && saTakerGets.issue().account == id ? tecOVERISSUE :
	       tecUNFUNDED_OFFER;
    }
    ...
    else if (!hasTakerPay(ctx.view, id, saTakerPays, ctx.j))
    {
        return tecUNFUNDED_DEAL_OFFER;
    }
...
CreateOffer::dry_offer (ApplyView& view, Offer const& offer)
{
    ...
    auto const amount = accountFunds(view, offer.owner(),
        offer.amount().out, fhZERO_IF_FROZEN, j);
    if (amount <= zero)
    {
        return true;
    }

    return !hasTakerPay (view, offer.owner(), offer.amount().in, j);
}
...
CreateOffer::reachedOfferCrossingLimit (Taker const& taker) const
{
    ...
    // The crossing limit is part of the MTChain protocol and
    ...
    NetClock::time_point const when)
{
    ...
    assert (!isM (taker_amount.in) && !isM (taker_amount.out));

    if (isM (taker_amount.in) || isM (taker_amount.out))
        Throw<std::logic_error> ("Bridging with M and an endpoint.");
    ...
    OfferStream offers_leg1 (view, view_cancel,
        Book (taker.issue_in (), mIssue ()),
        when, stepCounter_, j_);

    OfferStream offers_leg2 (view, view_cancel,
        Book (mIssue (), taker.issue_out ()),
        when, stepCounter_, j_);
    ...
    while (have_direct || have_bridge)
    {
        ...
        if (use_direct)
        {
            ...
            {
                auto const& offer = offers_direct.tip ();
                if (offer.getFee().signum () != 0)
                {
                    addTxFee (ctx_.tx, offer.getFee(), &offer.owner());
                }
            }
        ...
        else
        {
            ...
            if (cross_result == tesSUCCESS)
            {
                auto const& offer1 = offers_leg1.tip ();
                if (offer1.getFee().signum () != 0)
                {
                    addTxFee (ctx_.tx, offer1.getFee(), &offer1.owner());
                }

                auto const& offer2 = offers_leg2.tip ();
                if (offer2.getFee().signum () != 0)
                {
                    addTxFee (ctx_.tx, offer2.getFee(), &offer2.owner());
                }
            }
    ...
    Amounts const& taker_amount)
{
    ...
    //
    // We check this in preclaim, but when selling M
    ...
    if (taker.unfunded ())
    {
        ...
        return { !isM(taker_amount.in) && taker_amount.in.issue().account == account_ ?
                 tecOVERISSUE : tecUNFUNDED_OFFER, taker_amount };
    }

    try
    {
        auto cross_offer = &CreateOffer::bridged_cross;

        if (cross_type_ != CrossType::IouToIou)
            cross_offer = &CreateOffer::direct_cross;

        auto result = (this->*cross_offer)(taker, view, cancel_view, when);
        fee_ = taker.getFee ();
        totalFee_ = taker.getTotalFee ();
        return result;
    }
...
CreateOffer::preCompute()
{
    ...
    if (pays_m && !gets_m)
        cross_type_ = CrossType::IouToM;
    else if (gets_m && !pays_m)
        cross_type_ = CrossType::MToIou;
...
CreateOffer::applyGuts (ApplyView& view, ApplyView& view_cancel)
{
    ...
    if (result == tesSUCCESS)
    {
        ...
        taker_amount = make_Amounts (saTakerGets, saTakerPays);
    ...
    {
        MAmount reserve = ctx_.view().fees().accountReserve(
    ...
    if (result == tesSUCCESS)
    {
        ...
        if (result == tesSUCCESS)
        {
            ...
            sleOffer->setFieldAmount (sfTakerPaysTotal, taker_amount.out);
            sleOffer->setFieldAmount (sfTakerGetsTotal, taker_amount.in);
            if (totalFee_.signum () != 0)
            {
                sleOffer->setFieldAmount (sfTransferFee, totalFee_);
            }
            if (expiration)
