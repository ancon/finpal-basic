...
#include <mtchain/app/ledger/impl/TransactionAcquire.h>
...
void LedgerConsensusImp<Traits>::checkLCL ()
{
    ...
    if (netLgr != prevLedgerHash_)
    {
        ...
        JLOG (j_.warn()) << prevLedgerHash_
            << " to " << netLgr;
        JLOG (j_.warn())
            << mtchain::getJson (*previousLedger_);
...
void LedgerConsensusImp<Traits>::statePreClose ()
{
    ...
    int proposersValidated
        = app_.getValidations ().getNodesAfter//getTrustedValidationCount
          (prevLedgerHash_);
    ...
    milliseconds sinceClose;
    {
        ...
        auto closeTime = previousCloseCorrect
            ...
            : roundCloseTime(consensus_.getLastCloseTime(), closeResolution_); // use the time we saw

        auto now = roundCloseTime(app_.timeKeeper().closeTime(), closeResolution_);
        if (now >= closeTime)
...
void LedgerConsensusImp<Traits>::accept (TxSet_t const& set)
{
    ...
    std::shared_ptr<Ledger const> sharedLCL;
    {
...

        {
            ...
            //transferAmount(app_, secondAccount(), 50 * SYSTEM_CURRENCY_PARTS, accum);
