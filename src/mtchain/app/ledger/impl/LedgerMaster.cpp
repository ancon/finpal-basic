...
LedgerMaster::LedgerMaster (Application& app, Stopwatch& stopwatch,
    ...
    , fetch_seq_ (0)
    , last_missing_ledger_ (RangeSet::absent)
    , tryAdvanceTimer_ (this)
{
    tryAdvanceTimer_.setExpiration(10s);
}
...
*/
    {
        ScopedLockType sl (mCompleteLock);
        minVal = mCompleteLedgers.getFirst ();
        maxVal = mCompleteLedgers.getLast ();
    }

    if (maxVal == RangeSet::absent)
    {
        maxVal = mPubLedgerSeq.load();
        if (!maxVal) return false;
    }

    if (minVal == RangeSet::absent) minVal = maxVal;
    ...
    if (refHash)
    {
        ...
        if (!ledger)
        {
            auto const ledger = app_.getInboundLedgers().acquire (
                                     *refHash, refIndex, InboundLedger::/*fcHISTORY*/fcGENERIC);
            if (ledger)
            {
                ledgerHash = hashOfSeq(*ledger, index, m_journal);
                assert (ledgerHash);
...
void LedgerMaster::doAdvance ()
{
    ...
    std::uint32_t missing = last_missing_ledger_, last_missing;
    ...
    do
    {
        ...
        if (pubLedgers.empty())
        {
		    ...
		    last_missing = missing;
                    missing = mCompleteLedgers.prevMissing(missing == RangeSet::absent ?
                                                           mPubLedger->info().seq : missing);
                }
                JLOG (m_journal.trace())
                        ...
                        << "advanceThread should acquire";
                    {
                        ...
                        if (hash)
                        {
                            ...
                            if (!ledger)
                            {
                                        ...
                                        *hash))
                                {
                                    ...
                                    if (!ledger)  missing = last_missing;
                                }
                                else
                                    JLOG (m_journal.debug()) <<
                        ...
                        else
                        {
                            JLOG (m_journal.trace()) <<
                                ...
                                missing;
                            JLOG (m_journal.trace()) << "Pub:" <<
                                mPubLedgerSeq << " Val:" << mValidLedgerSeq;
                            JLOG (m_journal.trace()) << "Ledgers: " <<
        ...
        else
        {
            ...
            //missing_count += pubLedgers.size();
        }
        if (progress)
        {
            if (missing_count < ledger_fetch_size_)
            {
                mAdvanceWork = true;
            }
            else
            {
                //ScopedUnlockType sul(m_mutex);
                //tryAdvanceTimer_.setExpiration(3s);
            }
        }
    } while (mAdvanceWork);

    if (missing < last_missing_ledger_ || missing == RangeSet::absent)
    {
        last_missing_ledger_ = missing;
    }
}
