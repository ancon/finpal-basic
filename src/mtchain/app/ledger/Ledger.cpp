    ...
    , rules_{config.features}
{
    ...
    //int ratios[] = { 51, 5, 39, 5 };
    std::int64_t drops = 0;
    for (auto i = 0; i < config.MINERS.size(); ++i)
    {
        auto const& id = config.MINERS[i].first;
        auto const ratio = config.MINERS[i].second;
        auto const sle = std::make_shared<SLE>(keylet::account(id));

        sle->setFieldU32 (sfSequence, 1);
        sle->setAccountID (sfAccount, id);
        auto balance = info_.drops.drops() / 100 * ratio;
        sle->setFieldAmount (sfBalance, balance);
        drops += balance;
        rawInsert(sle);
    }

    if (drops > info_.drops.drops())
    {
        info_.drops = drops;
        drops = 0;
    }
    ...
    {
        drops = info_.drops.drops() - drops; 
    }

    {
        auto const& id = genesisAccount();
        auto const sle = std::make_shared<SLE>(keylet::account(id));
        sle->setFieldU32 (sfSequence, 1);
        sle->setAccountID (sfAccount, id);
    ...
    #define FIRST_MESSAGE "A bufferfly in the tropical rainforest of the Amazon River Basin " \
        "in South America, occasionally flapping its wings, can cause a tornado in Texas " \
        "in the United States two weeks later."

    memo.setFieldVL(sfMemoType, to_blob(strHex("FirstMessage")));
    memo.setFieldVL(sfMemoFormat, to_blob(strHex("TEXT")));
    memo.setFieldVL(sfMemoData, to_blob(strHex(FIRST_MESSAGE, strlen(FIRST_MESSAGE))));

    transferAmount(app, firstAccount(), 441000 * SYSTEM_CURRENCY_PARTS, view, &memo);
    transferAmount(app, secondAccount(), 50 * SYSTEM_CURRENCY_PARTS, view);
    view.apply(*this);
    */

    stateMap_->flushDirty (hotACCOUNT_NODE, info_.seq);
    //txMap_->flushDirty (hotTRANSACTION_NODE, info_.seq);
    setImmutable(config);
}
    ...
    , rules_(prevLedger.rules_)
{
    ...
    info_.feePool = SYSTEM_CURRENCY_START - info_.drops;
