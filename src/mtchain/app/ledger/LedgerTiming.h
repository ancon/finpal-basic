...
#ifndef MTCHAIN_APP_LEDGER_LEDGERTIMING_H_INCLUDED
#define MTCHAIN_APP_LEDGER_LEDGERTIMING_H_INCLUDED
...
/** Calculates the close time resolution for the specified ledger.
    ...
    The MTChain protocol uses binning to represent time intervals using only one
...
auto constexpr LEDGER_IDLE_INTERVAL = 10s;//15s; //10min;
...
auto constexpr VALIDATION_VALID_WALL = 5min; //15min;
...
auto constexpr VALIDATION_VALID_LOCAL = 3min; //13min;

// The number of seconds before a close time that we consider a validation
// acceptable. This protects against extreme clock errors
auto constexpr VALIDATION_VALID_EARLY = 3min; //13min;
...
auto constexpr LEDGER_GRANULARITY = 10s; //10min; // change ledger closingtime to from 1s to 10s

// How long we consider a proposal fresh
auto constexpr PROPOSE_FRESHNESS = 20s; //20min;

// How often we force generating a new proposal to keep ours fresh
auto constexpr PROPOSE_INTERVAL = 12s; //12min;
...
auto constexpr AV_MIN_CONSENSUS_TIME = 5s; //5min;
