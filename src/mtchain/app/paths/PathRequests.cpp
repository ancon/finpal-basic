...
/** Get the current MTChainLineCache, updating it if necessary.
    Get the correct ledger to use.
*/
std::shared_ptr<MTChainLineCache>
    ...
    bool authoritative)
{
    ...
    {
        mLineCache = std::make_shared<MTChainLineCache> (ledger);
    }
    return mLineCache;
}
                              ...
                              Job::CancelCallback shouldCancel)
{
    ...
    std::shared_ptr<MTChainLineCache> cache;
...
// Make an old-style mtchain_path_find request
Json::Value
...
PathRequests::doLegacyPathRequest (
        ...
        Json::Value const& request)
{
    auto cache = std::make_shared<MTChainLineCache> (inLedger);
