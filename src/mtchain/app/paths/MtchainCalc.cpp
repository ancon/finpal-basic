...
#include <mtchain/app/paths/impl/FlowDebugInfo.h>
...
MtchainCalc::Output MtchainCalc::mtchainCalculate (
    PaymentSandbox& view,
    ...
    Input const* const pInputs)
{
    ...
    if (callFlowV1)
    {
        ...
        auto result = rc.mtchainCalculate (compareFlowV1V2 ? &flowV1FlowDebugInfo : nullptr);
    ...
    if (j.debug())
    {
        ...
        auto logResult = [&](std::string const& algoName,
            ...
            j.debug () << systemName() + "Calc Result> " <<
...
bool MtchainCalc::addPathState(STPath const& path, TER& resultCode)
{
    ...
    if (pathState->status() == tesSUCCESS)
        pathState->checkNoMTChain (uDstAccountID_, uSrcAccountID_);
...
TER MtchainCalc::mtchainCalculate (detail::FlowDebugInfo* flowDebugInfo)
{
    JLOG (j_.trace())
        << systemName() + "Calc>"
    ...
    {
        JLOG (j_.debug())
	    << systemName() + "Calc: Invalid transaction:"
            << "No paths and direct mtchain not allowed.";

        return temMTCHAIN_EMPTY;
    }

    // Build a default path.  Use saDstAmountReq_ and saMaxAmountReq_ to imply
    // nodes.
    // XXX Might also make a M bridge by default.

    JLOG (j_.trace())
        << systemName() + "Calc: Paths in set: " << spsPaths_.size ();
        ...
        for (auto pathState : pathStateList_)
        {
                ...
                // Only do active paths.
            {
                ...
                else if (pathState->outPass() == zero)
                {
                    ...
                    JLOG (j_.warn())
                        << systemName() + "Calc: Non-dry path moves no funds";
