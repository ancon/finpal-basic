...
TER PathCursor::advanceNode (STAmount const& amount, bool reverse, bool callerHasLiquidity) const
{
    ...
    PathCursor withMultiQuality {mtchainCalc_, pathState_, multi, j_, nodeIndex_};
        ...
        else
        {
            ...
            else
            {
                            ...
                            view().parentCloseTime().time_since_epoch().count()))
                {
                    ...
                    mtchainCalc_.permanentlyUnfundedOffers_.insert(
                        node().offerIndex_);
                    continue;
                }
                ...
                if (node().saTakerPays <= zero || node().saTakerGets <= zero)
                {
                    ...
                    else if (mtchainCalc_.permanentlyUnfundedOffers_.find (index)
                             != mtchainCalc_.permanentlyUnfundedOffers_.end ())
                    {
                ...
                auto itPast = mtchainCalc_.mumSource_.find (accountIssue);
                bool bFoundPast = (itPast != mtchainCalc_.mumSource_.end ());
                ...
                if (node().saOfferFunds <= zero)
                {
                    ...
                    if (bReverse && !bFoundReverse && !bFoundPast)
                    {
                        ...
                        // still do deletions.
                        // Mark offer for always deletion.
                        mtchainCalc_.permanentlyUnfundedOffers_.insert (node().offerIndex_);
                    }
                    else
                    {
