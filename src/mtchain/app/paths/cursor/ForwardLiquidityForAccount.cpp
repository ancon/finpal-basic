...
// - If next node is an offer and output is non-M then we are the issuer and
//   do not need to push funds.
// - If next node is an offer and output is M then we need to deliver funds to
//   limbo.
TER PathCursor::forwardLiquidityForAccount () const
{
    TER resultCode   = tesSUCCESS;
    auto const lastNodeIndex       = pathState_.nodes().size () - 1;
    auto viewJ = mtchainCalc_.logs_.journal ("View");
    ...
    if (previousNode().isAccount() && nextNode().isAccount())
    {
        ...
        if (!nodeIndex_)
        {
            ...
            // For the first node, calculate amount to mtchain based on what is
            // available.
    ...
    else if (previousNode().isAccount() && !nextNode().isAccount())
    {
        ...
        if (nodeIndex_)
        {
            // Non-M, current node is the issuer.
            JLOG (j_.trace())
                << "forwardLiquidityForAccount: account --> "
                << "ACCOUNT --> offer";
        ...
        else
        {
            ...
            if (pathState_.inReq() >= zero)
            {
                ...
                // Limit M by available. No limit for non-M as issuer.
                if (isM (node().issue_))
                    node().saFwdDeliver = std::min (
                        node().saFwdDeliver,
                        accountHolds(view(),
                            ...
                            mCurrency(),
                            mAccount(),
                            fhIGNORE_FREEZE, viewJ)); // M can't be frozen
            ...
            else if (!isM (node().issue_))
            {
                // Non-M, current node is the issuer.
                // We could be delivering to multiple accounts, so we don't know
                // which mtchain balance will be adjusted.  Assume just issuing.
            ...
            else
            {
                ...
                resultCode = accountSend(view(),
                    node().account_, mAccount(), node().saFwdDeliver, viewJ);
            }
        }
    }
    else if (!previousNode().isAccount() && nextNode().isAccount())
