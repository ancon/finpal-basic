    ...
    while (resultCode == tesSUCCESS && saInAct + saInFees < saInReq)
    {
        ...
        else if (resultCode == tesSUCCESS)
        {
            ...
            if (!isM(nextNode().account_))
            {
                ...
                // Output fees: none as M or the destination account is the
                // issuer.
                ...
                // Output: Debit offer owner, send M or non-XPR to next
                // account.
            ...
            else
            {
                ...
                // fees).
                auto const& id = isM(node().issue_) ?
                        mAccount() : node().issue_.account;
            ...
            // is redundant.
            if (isM (previousNode().issue_.currency)
                || uInAccountID != node().offerOwnerAccount_)
            {
                auto id = !isM(previousNode().issue_.currency) ?
                        uInAccountID : mAccount();
                resultCode = accountSend(view(),
                    id,
                    node().offerOwnerAccount_,
                    saInPassAct,
