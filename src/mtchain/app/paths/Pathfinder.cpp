...
The pathfinding request is identified by category, M to M, M to
non-M, non-M to M, same currency non-M to non-M, cross-currency
non-M to non-M.  For each category, there is a table of paths that the
...
Pathfinder::Pathfinder (
    ...
    :   mSrcAccount (uSrcAccount),
        ...
        mEffectiveDst (isM(saDstAmount.getIssuer ()) ?
        ...
        mSrcAmount (srcAmount.value_or(STAmount({uSrcCurrency,
            uSrcIssuer.value_or(isM(uSrcCurrency) ?
                mAccount() : uSrcAccount)}, 1u, 0, true))),
...
{
    assert (! uSrcIssuer || isM(uSrcCurrency) == isM(uSrcIssuer.get()));
}
...
bool Pathfinder::findPaths (int searchLevel)
{
    ...
    auto currencyIsM = isM (mSrcCurrency);

    bool useIssuerAccount
            = mSrcIssuer && !currencyIsM && !isM (*mSrcIssuer);
    auto& account = useIssuerAccount ? *mSrcIssuer : mSrcAccount;
    auto issuer = currencyIsM ? AccountID() : account;
    ...
    bool bSrcM = isM (mSrcCurrency);
    bool bDstM = isM (mDstAmount.getCurrency());
    ...
    if (! mLedger->exists (keylet::account (mDstAccount)))
    {
        ...
        {
            JLOG (j_.debug())
                    << "New account not being funded in M ";
            return false;
        }
    ...
    {
        // M -> M
        JLOG (j_.debug()) << "M to M payment";
        paymentType = pt_M_to_M;
    }
    else if (bSrcM)
    {
        // M -> non-M
        JLOG (j_.debug()) << "M to non-M payment";
        paymentType = pt_M_to_nonM;
    }
    else if (bDstM)
    {
        // non-M -> M
        JLOG (j_.debug()) << "non-M to M payment";
        paymentType = pt_nonM_to_M;
    }
    else if (mSrcCurrency == mDstAmount.getCurrency ())
    {
        // non-M -> non-M - Same currency
        JLOG (j_.debug()) << "non-M to non-M - same currency";
        paymentType = pt_nonM_to_same;
    }
    else
    {
        // non-M to non-M - Different currency
        JLOG (j_.debug()) << "non-M to non-M - cross currency";
        paymentType = pt_nonM_to_nonM;
    }
    ...
    uint64_t& qualityOut) const    // OUT: The returned initial quality
{
    ...
    try
    {
        ...
        auto rc = path::MtchainCalc::mtchainCalculate (
            sandbox,
            mSrcAmount,
            minDstAmount,
            mDstAccount,
...
static bool isDefaultPath (STPath const& path)
{
    ...
    // https://forum.mtchain.io/viewtopic.php?f=2&t=8206&start=10#p57713
    //
    // JoelKatz writes:
    ...
    AccountID const& srcIssuer)
{
    ...
    const bool issuerIsSender = isM (mSrcCurrency) || (srcIssuer == mSrcAccount);
    ...
    AccountID const& dstAccount)
{
    ...
    if (!bFrozen)
    {
        ...
        for (auto const& item : mRLCache->getMTChainLines (account))
        {
            MTChainState* rspEntry = (MTChainState*) item.get ();
            ...
            else if (rspEntry->getNoMtchainPeer ())
            {
...
STPathSet& Pathfinder::addPathsForType (PathType const& pathType)
{
    ...
    case nt_M_BOOK:
        addLinks (parentPaths, pathsOut, afADD_BOOKS | afOB_M);
        break;
...
bool Pathfinder::isNoMTChain (
    AccountID const& fromAccount,
    AccountID const& toAccount,
    Currency const& currency)
{
    auto sleMTChain = mLedger->read(keylet::line(
        toAccount, fromAccount, currency));

    auto const flag ((toAccount > fromAccount)
                     ? lsfHighNoMtchain : lsfLowNoMtchain);

    return sleMTChain && (sleMTChain->getFieldU32 (sfFlags) & flag);
}

// Does this path end on an account-to-account link whose last account has
// set "no mtchain" on the link?
bool Pathfinder::isNoMTChainOut (STPath const& currentPath)
{
    ...
    // no mtchain on the output. A path with no mtchain on its output can't be
    // followed by a link with no mtchain on its input.
    auto const& fromAccount = (currentPath.size () == 1)
        ? mSrcAccount
        : (currentPath.end () - 2)->getAccountID ();
    auto const& toAccount = endElement.getAccountID ();
    return isNoMTChain (fromAccount, toAccount, endElement.getCurrency ());
}
    ...
    int addFlags)
{
    ...
    bool const bOnM = uEndCurrency.isZero ();
    ...
    if (addFlags & afADD_ACCOUNTS)
    {
        ...
        if (bOnM)
        {
            ...
            { // non-default path to M destination
                JLOG (j_.trace())
        ...
        else
        {
            ...
            if (sleEnd)
            {
                ...
                bool const bIsNoMTChainOut (
                    isNoMTChainOut (currentPath));
                bool const bDestOnly (
                    addFlags & afAC_LAST);

                auto& mtchainLines (mRLCache->getMTChainLines (uEndAccount));

                AccountCandidates candidates;
                candidates.reserve (mtchainLines.size ());

                for (auto const& item : mtchainLines)
                {
                    auto* rs = dynamic_cast<MTChainState const *> (item.get ());
                    if (!rs)
                    {
                        JLOG (j_.error())
                                << "Couldn't decipher MTChainState";
                        continue;
                    }
                        ...
                        !currentPath.hasSeen (acct, uEndCurrency, acct))
                    {
                        ...
                        else if (bIsNoMTChainOut && rs->getNoMtchain ())
                        {
    ...
    if (addFlags & afADD_BOOKS)
    {
        ...
        {
            // to M only
            if (!bOnM && app_.getOrderBookDB ().isBookToM (
            ...
            for (auto const& book : books)
            {
                     ...
                     (book->getCurrencyOut () == mDstAmount.getCurrency ())))
                {
                    ...
                    { // to M
                        ...
                        {
                            // destination is M, add account and path is
                            // complete
                            JLOG (j_.trace())
...
Pathfinder::PathType makePath (char const *string)
{
    ...
    while (true)
    {
        switch (*string++)
        {
            ...
            case 'x': // m book
                ret.push_back (Pathfinder::nt_M_BOOK);
                break;
...
void Pathfinder::initPathTable ()
{
    ...
    fillPaths(
        pt_nonM_to_M, {
            {1, "sxd"},       // gateway buys M
            {2, "saxd"},      // source -> gateway -> book(M) -> dest
            {6, "saaxd"},
            {7, "sbxd"},
            {8, "sabxd"},
            {9, "sabaxd"}
    ...
    fillPaths(
        pt_nonM_to_same,  {
            ...
            {6, "sxfad"},
            {6, "safad"},
            {6, "saxfd"},   // source -> gateway -> book to M -> book ->
                            // destination
            {6, "saxfad"},
