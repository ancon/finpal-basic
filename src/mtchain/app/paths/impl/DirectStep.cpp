    ...
    Currency const& cur)
{
    ...
    auto const limit = creditLimit2 (sb, dst, src, cur);

    // srcOwed is negative or zero
    return {limit + srcOwed, limit.signum () <= 0};
}
    ...
    IOUAmount const& out)
{
    ...
    if (maxSrcToDst.signum () <= 0)
    {
        ...
        cache_.emplace (
            ...
            IOUAmount (beast::zero),
            srcRedeems);
        sb.result = srcRedeems ? tecUNFUNDED_PAYMENT_IOU : tecEXCEED_TRUST_LIMIT;
...
TER DirectStepI::check (StrandContext const& ctx) const
{
    ...
    if (ctx.prevStep)
    {
        ...
        {
            auto const ter =
                checkNoMTChain (ctx.view, *prevSrc, src_, dst_, currency_, j_);
            if (ter != tesSUCCESS)
                return ter;
        }
        ...
        if (fix1449(ctx.view.info().parentCloseTime))
        {
            ...
            {
                auto const noMTChainSrcToDst =
                    ((*sleLine)[sfFlags] &
                     ((src_ > dst_) ? lsfHighNoMtchain : lsfLowNoMtchain));
                if (noMTChainSrcToDst)
