...
static
bool isMAccount (STPathElement const& pe)
{
    if (pe.getNodeType () != STPathElement::typeAccount)
        return false;
    return isM (pe.getAccountID ());
};
    ...
    Issue const& curIssue)
{
    ...
    if (ctx.isFirst && e1->isAccount () &&
        ...
        isM (e1->getCurrency ()))
    {
        return make_MEndpointStep (ctx, e1->getAccountID ());
    }

    if (ctx.isLast && isMAccount (*e1) && e2->isAccount())
        return make_MEndpointStep (ctx, e2->getAccountID());
    ...
    if (isM (curIssue.currency) && isM (outCurrency))
    {
        JLOG (j.warn()) << "Found m/m offer payment step";
    ...
    beast::Journal j)
{
    if (isM (src))
    {
        JLOG (j.debug()) << "toStrand with mAccount as src";
        return {temBAD_PATH, Strand{}};
    }
    if (isM (dst))
    {
        JLOG (j.debug()) << "toStrand with mAccount as dst";
        return {temBAD_PATH, Strand{}};
    }
    ...
    beast::Journal j)
{
    ...
    {
        JLOG (j.debug())
            << "Flow: Invalid transaction: No paths and direct mtchain not allowed.";
        return {temMTCHAIN_EMPTY, std::vector<Strand>{}};
    }
...
template<>
bool
isDirectMToM<MAmount, MAmount> (Strand const& strand)
{
...
template
bool
isDirectMToM<MAmount, IOUAmount> (Strand const& strand);
template
bool
isDirectMToM<IOUAmount, MAmount> (Strand const& strand);
template
bool
isDirectMToM<IOUAmount, IOUAmount> (Strand const& strand);
