    ...
    beast::Journal j)
{
    ...
    auto rate = [&](Issue const& issue)->std::uint32_t
    {
        if (isM (issue.account) || issue.account == dst)
            return QUALITY_ONE;
        return transferRate (sb, issue.account, issue.currency).value;
    };
...
template <class TIn, class TOut>
static
bool equalHelper (Step const& step, mtchain::Book const& book)
{
...
{
    bool const inM = isM (book.in.currency);
    bool const outM = isM (book.out.currency);
    if (inM && outM)
        return equalHelper<MAmount, MAmount> (step, book);
    if (inM && !outM)
        return equalHelper<MAmount, IOUAmount> (step, book);
    if (!inM && outM)
        return equalHelper<IOUAmount, MAmount> (step, book);
    if (!inM && !outM)
...
{
    Issue out;
    return make_BookStepHelper<IOUAmount, MAmount> (ctx, in, out);
}
...
{
    Issue in;
    return make_BookStepHelper<MAmount, IOUAmount> (ctx, in, out);
}
