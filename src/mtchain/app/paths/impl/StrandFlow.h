...
#ifndef MTCHAIN_APP_PATHS_IMPL_STRANDFLOW_H_INCLUDED
#define MTCHAIN_APP_PATHS_IMPL_STRANDFLOW_H_INCLUDED
...
/*
   ...
   @param strand Steps of Accounts to mtchain through and offer books to use
    ...
    beast::Journal j)
{
    ...
    if (isDirectMToM<TInAmt, TOutAmt> (strand))
    {
        // The current implementation returns NO_LINE for M->M transfers.
        // Keep this behavior
    ...
    try
    {
        ...
        EitherAmount limitStepOut;
        {
            ...
            for (auto i = s; i--;)
            {
                ...
                if (sb->result != tesSUCCESS)
                {
                    baseView.result = sb->result;
                }
...
/*
   ...
   @param baseView Trust lines and balances
   @param strands Each strand contains the steps of accounts to mtchain through
    ...
    path::detail::FlowDebugInfo* flowDebugInfo=nullptr)
{
    ...
    if (actualOut != outReq)
    {
        ...
        if (!partialPayment)
        {
            return {sb.result != tesSUCCESS ? sb.result : tecPATH_PARTIAL,
