...
bool PathRequest::isValid (std::shared_ptr<MTChainLineCache> const& crCache)
{
    ...
    if (! sleDest)
    {
        ...
        {
            // Only M can be send to a non-existent account.
    ...
    {
        bool const disallowM (
            sleDest->getFlags() & lsfDisallowM);

        auto usDestCurrID = accountDestCurrencies (
                *raDstAccount, crCache, ! disallowM);
...
PathRequest::getPathFinder(std::shared_ptr<MTChainLineCache> const& cache,
...
bool
PathRequest::findPaths (std::shared_ptr<MTChainLineCache> const& cache,
    int const level, Json::Value& jvArray)
{
        ...
        for (auto const& c : currencies)
        {
            if (! sameAccount || c != saDstAmount.getCurrency())
            {
                ...
                sourceCurrencies.insert(
                    {c, c.isZero() ? mAccount() : *raSrcAccount});
            }
        }
    }
    ...
    for (auto const& issue : sourceCurrencies)
    {
        ...
        auto& sourceAccount = ! isM(issue.account)
            ? issue.account
            : isM(issue.currency)
            ? mAccount()
            : *raSrcAccount;
        ...
        JLOG(m_journal.debug()) << iIdentifier
            << " Paths found, calling mtchainCalc";
        ...
        if (rc.result () == tesSUCCESS)
        {
            ...
            if (hasCompletion ())
            {
                // Old mtchain_path_find API requires this
        ...
        {
            JLOG(m_journal.debug()) << iIdentifier << " mtchainCalc returns "
                << transHuman(rc.result());
        }
    }
...
Json::Value PathRequest::doUpdate(
    std::shared_ptr<MTChainLineCache> const& cache, bool fast)
{
    using namespace std::chrono;
    JLOG(m_journal.debug()) << iIdentifier
    ...
    if (hasCompletion ())
    {
        // Old mtchain_path_find API gives destination_currencies
