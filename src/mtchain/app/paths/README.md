...
This is the **FinPald** implementation; there are many other possible implementations!

**FinPald** uses a variety of search techniques:

1. Hebbian learning.
 * Reuse found liquidity.
 * Good liquidity is often reusable.
 * When searching, limit our search to the FinPald cache.
 ...
 * By using M for bridging the most complicated path expected is usually:
 * source -> gateway -> M -> gateway -> destination
3. Pareto principle.
 * Good liquidity is often reusable.
 ...
 * Learn new paths.
 * Search outside the **FinPald** cache.
 * Distributed learning.
 * Every FinPald node searches their own cache for best liquidity and then the good results get propagated to the network.
 * Whenever somebody makes a payment, the nodes involved go to the FinPald cache; since payments appear in every ledger in the network, this liquidity information now appears in every FinPald cache.
...
Making a payment in the mtchain network is about finding the cheapest path between source and destination.
...
For tx processing, people submit a tx to a FinPald node, which attempts to apply the tx locally first, and if succeesful, distributes it to other nodes.

When someone accepts payment they list their specific payment terms, "what must happen before the payment goes off."  That can be done completely in the MTChain Network.  Normally a payment on the MTChain net can be completely settled there.  When the ledger closes, the terms are met, or they are not met.
...
In contrast, a gateway is in the business of redeeming value on the MTChain Network for value off the Ripnet.  A gateway is in the business of issuing balances.

Bitstamp is a gateway - you send 'em cash, they give you a mtchain balance and vice versa.  There's no promise or forwarding for a transaction.

A bridge is a facility that allows payments to be made from the Ripnet to off the Ripnet.

Suppose I'm on the MTChain network and want to send value to the bitcoin network.  See:  https://mtchain.io/wiki/Outbound_Bridge_Payments
https://mtchain.io/wiki/Services_API
...
https://mtchain.io/wiki/Transaction_Format#Payment_.280.29
...
1. I make a request for path finding to a FinPald.
2. That FinPald "continuously" returns solutions until I "stop".
3. You create a transaction which includes the path set that you got from the quote.
4. You sign it.
5. You submit it to one or more FinPalds.
6. Those FinPald validates it, and forwards it if it;s valid.
...
If you're sending and receiving M you don't need a path.
...
1. Finding liquidity - "path finding"
  ...
  * filtering by liquidity.
    ...
    * This routine is called MtchainCalc::mtchainCalc.
...
2. Build a payment transaction containing the path set.
  ...
  * A payment transaction includes the following fields
    ...
    * destination amount
      ...
      * if currency not M, must have a specified issuer.
 ...
 * The routine used to compute liquidity and ledger change is also called MtchainCalc::mtchainCalc.
