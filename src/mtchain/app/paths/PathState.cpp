...
// - A mtchain nodes output issuer must be the node's account or the next node's
//   account.
// - Offers can only go directly to another offer if the currency and issuer are
//   an exact match.
// - Real issuers must be specified for non-M.
    ...
    AccountID const& issuer)    // --> Delivering this issuer.
{
    ...
    if (nodes_.back ().issue_.currency != currency)
    {
        ...
        // book.  mAccount() does double duty as signaling "this is an order
        // book".

        // Corresponds to "Implies an offer directory" in the diagram, currently
        // at http://goo.gl/Uj3HAB.

        auto type = isM(currency) ? STPathElement::typeCurrency
            : STPathElement::typeCurrency | STPathElement::typeIssuer;

        // The offer's output is what is now wanted.
        // mAccount() is a placeholder for offers.
        resultCode = pushNode (type, mAccount(), currency, issuer);
    }


    // For mtchain, non-M, ensure the issuer is on at least one side of the
    // transaction.
    ...
    {
        // Need to mtchain through issuer's account.
    ...
    AccountID const& issuer)     // If not specified, default to previous.
{
    ...
    // true, iff node is a mtchain account. false, iff node is an offer node.
    ...
    else if (hasIssuer && isM (node.issue_))
    {
        JLOG (j_.debug()) << "pushNode: issuer specified for M.";
    ...
    else if (hasAccount)
    {
        ...
        node.issue_.account = hasIssuer ? issuer :
                (isM (node.issue_) ? mAccount() : account);
        ...
        else
        {
            ...
            resultCode = pushImpliedNodes (
                ...
                isM(node.issue_.currency) ? mAccount() : account);
        ...
        if (resultCode == tesSUCCESS && !nodes_.empty ())
        {
            ...
            {
                auto sleMtchainState = view().peek(
                    keylet::line(backNode.account_, node.account_, backNode.issue_.currency));

                // A "MTChainState" means a balance betweeen two accounts for a
                // specific currency.
                if (!sleMtchainState)
                {
                    JLOG (j_.trace())
                ...
                else
                {
                    ...
                    if (!sleBck)
                    {
                        ...
                        resultCode   = terNO_ACCOUNT;
                             ...
                             !(sleMtchainState->getFieldU32 (sfFlags) &
                                  (bHigh ? lsfHighAuth : lsfLowAuth)) &&
                             sleMtchainState->getFieldAmount(sfBalance) == zero)
                    {
                        JLOG (j_.warn())
                                << "pushNode: delay: can't receive IOUs from "
                                << "issuer without auth.";
    ...
    else
    {
        ...
        else if (isM (node.issue_.currency))
            node.issue_.account = mAccount();
        else if (isM (backNode.issue_.account))
        ...
        else if (backNode.issue_ == node.issue_)
        {
            ...
            resultCode   = pushImpliedNodes (
                mAccount(), // Rippling, but offers don't have an account.
    ...
    AccountID const& uSenderID)
{
    ...
    AccountID const& uSenderIssuerID
        = isM(uMaxCurrencyID) ? mAccount() : uSenderID;
    // Sender is always issuer for non-M.
    ...
    // M with issuer is malformed.
    if ((isM (uMaxCurrencyID) && !isM (uMaxIssuerID))
        || (isM (currencyOutID) && !isM (issuerOutID)))
    {
        JLOG (j_.debug())
            << "expandPath> issuer with M";
        terStatus   = temBAD_PATH;
    }

    // Push sending node.
    // For non-M, issuer is always sending account.
    ...
    if (tesSUCCESS == terStatus && uMaxIssuerID != uSenderIssuerID)
    {
        ...
        // - If it was M, then issuers would have matched.
        ...
        // account.
        if (!uNxtCurrencyID
            // Next is M, offer next. Must go through issuer.
    ...
    if (terStatus == tesSUCCESS
        && !isM(currencyOutID)               // Next is not M
...
/** Check if a sequence of three accounts violates the no mtchain constrains
    [first] -> [second] -> [third]
    Disallowed if 'second' set no mtchain on [first]->[second] and
    [second]->[third]
*/
TER PathState::checkNoMTChain (
    ...
    Currency const& currency)
{
    ...
    if (!sleIn || !sleOut)
    {
        ...
        sleIn->getFieldU32 (sfFlags) &
            ((secondAccount > firstAccount) ? lsfHighNoMtchain : lsfLowNoMtchain) &&
        sleOut->getFieldU32 (sfFlags) &
            ((secondAccount > thirdAccount) ? lsfHighNoMtchain : lsfLowNoMtchain))
    {
        JLOG (j_.info())
            << "Path violates noMTChain constraint between "
            << firstAccount << ", "
            << secondAccount << " and "
            << thirdAccount;

        terStatus = terNO_MTCHAIN;
    }
    return terStatus;
}

// Check a fully-expanded path to make sure it doesn't violate no-MTChain
// settings.
TER PathState::checkNoMTChain (
    AccountID const& uDstAccountID,
    AccountID const& uSrcAccountID)
{
    // There must be at least one node for there to be two consecutive mtchain
    // lines.
    if (nodes_.size() == 0)
       return terStatus;
    ...
    // These are the nodes whose no mtchain constraints could be violated
