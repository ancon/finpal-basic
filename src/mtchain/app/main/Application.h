...
#ifndef MTCHAIN_APP_MAIN_APPLICATION_H_INCLUDED
#define MTCHAIN_APP_MAIN_APPLICATION_H_INCLUDED
...
#ifdef IPFS_ENABLE
    virtual std::shared_ptr<ipfs::Client> createIpfsClient() = 0;
    virtual void setIpfsAddress (std::string const& ip, std::uint16_t port) = 0;
    virtual int storeFile (std::shared_ptr<ipfs::Client> client,
                           std::string const& path, std::string const& filename,
                           ipfs::Json &result) = 0;
    virtual AccountID const& getFileChargeAccount() = 0;
    virtual std::string const& getFileChargeFee(bool upload) = 0;
#endif
};
