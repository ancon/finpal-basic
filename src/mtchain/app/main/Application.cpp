...
#include <mtchain/app/main/DBInit.h>
#include <mtchain/app/main/BasicApp.h>
#include <mtchain/app/main/Tuning.h>
...
#include <mtchain/app/main/LoadManager.h>
...
#include <mtchain/app/misc/ValidatorSite.h>
...
#include <mtchain/basics/ResolverAsio.h>
...
#include <mtchain/overlay/make_Overlay.h>
...
#include <mtchain/beast/asio/io_latency_probe.h>
...
public:
...
#ifdef IPFS_ENABLE
    std::string m_ipfs_ip;
    std::uint16_t m_ipfs_port;
    std::unique_ptr <ipfs::Client> m_ipfsClient;
    boost::optional<AccountID> mFileChargeAccount;
    std::string mFileUploadFee;
    std::string mFileDownloadFee;
#endif
    ...
    {
    #if MTCHAIN_SINGLE_IO_SERVICE_THREAD
        return 1;
    #else
...
#ifdef IPFS_ENABLE
    std::shared_ptr<ipfs::Client> createIpfsClient () override
    {
        return  std::make_shared<ipfs::Client>(m_ipfs_ip, m_ipfs_port);
    }

    void setIpfsAddress (std::string const& ip, std::uint16_t port)
    {
        m_ipfs_ip = ip;
        m_ipfs_port = port;
    }

    int storeFile (std::shared_ptr<ipfs::Client> client,
                   std::string const& path, std::string const& filename,
                   ipfs::Json &result) override
    {
        try {
            client->FilesAdd({
                    { filename, ipfs::http::FileUpload::Type::kFileName, path }
                }, &result);
        } catch (std::exception const& e)
        {
            JLOG(m_journal.error()) << "update file '" << path << "' into ipfs failed: "
                                    << e.what();
            return -1;
        }
    ...
    AccountID const& getFileChargeAccount() override
    {
        return *mFileChargeAccount;
    }

    std::string const& getFileChargeFee(bool upload) override
    {
        return upload ? mFileUploadFee : mFileDownloadFee;
    }
#endif
...
//------------------------------------------------------------------------------
void transferAmount(Application& app, AccountID const& dstAccount, STAmount const& amount, OpenView &view, STObject *memo)
{
    AccountID const& srcAccount = genesisAccount();

    STObject txObj(sfTransaction);
    txObj.setFieldU16(sfTransactionType, ttPAYMENT);
    txObj.setAccountID(sfAccount, srcAccount);

    auto sle = view.read(keylet::account(srcAccount));
    txObj.setFieldU32(sfSequence, (*sle)[sfSequence]);

    txObj.setFieldAmount(sfFee, STAmount(view.fees().units));
    txObj.setFieldVL(sfSigningPubKey, Blob());

    txObj.setAccountID(sfDestination, dstAccount);
    txObj.setFieldAmount(sfAmount, amount);

    if (memo)
    {
        STArray memos;
        memos.push_back(*memo);
        txObj.setFieldArray(sfMemos, memos);
    }

    STTx tx(std::move(txObj));
    applyTransaction (app, view, tx, false, tapNO_CHECK_SIGN, app.journal("Mining"));
}
...
ApplicationImp::startGenesisLedger()
{
    ...
    auto const next = std::make_shared<Ledger>(
        *genesis, timeKeeper().closeTime() - genesis->info().closeTimeResolution);
    next->updateSkipList ();

    /*
    OpenView view(&*next);
    transferAmount(*this, secondAccount(), 50 * SYSTEM_CURRENCY_PARTS, view);
    view.apply(*next);
    */

    next->stateMap().flushDirty (hotACCOUNT_NODE, next->info().seq);
    //next->txMap().flushDirty (hotTRANSACTION_NODE, next->info().seq);
    ...
    std::string const& name)
{
    try
    {
        ...
        if (ledger.get().isMember ("accountState"))
        {
			...
			if (ledger.get().isMember("fee_pool"))
			{
				feePool =
					beast::lexicalCastThrow<std::uint64_t>
					(ledger.get()["fee_pool"].asString());
			}
...
void ApplicationImp::parseFileChargeInfo()
{
#ifdef IPFS_ENABLE
    if (config().exists(SECTION_IPFS))
    {
        auto const& section = config().section(SECTION_IPFS);

        std::string account;
        if (set(account, "account_id", section))
        {
            mFileChargeAccount = parseBase58<AccountID>(account);
        }

        if (set(mFileUploadFee, "storage_fee", section))
        {
            mFileUploadFee = "1/" + mFileUploadFee;
        }

        if (set(mFileDownloadFee, "download_fee", section))
        {
            mFileDownloadFee = "1/" + mFileDownloadFee;
        }
    }

    if (!mFileChargeAccount)
    {
        mFileChargeAccount = genesisAccount();
    }

    if (mFileUploadFee.empty())
    {
        mFileUploadFee = "1/1";
    }

    if (mFileDownloadFee.empty())
    {
        mFileDownloadFee = "1/1";
    }
#endif
}
