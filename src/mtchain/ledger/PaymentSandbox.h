...
#ifndef MTCHAIN_LEDGER_PAYMENTSANDBOX_H_INCLUDED
#define MTCHAIN_LEDGER_PAYMENTSANDBOX_H_INCLUDED
...
public:
    ...
    */
    ...
    // the total change in M balances.
    std::map<std::tuple<AccountID, AccountID, Currency>, STAmount>
    balanceChanges (ReadView const& view) const;

    MAmount mDestroyed () const;

    TER mutable result = tesSUCCESS; /* carry transaction result */
private:
