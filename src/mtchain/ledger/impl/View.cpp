...
{
    auto closeTimeRes = info.closeTimeResolution.count();
    if (closeTimeRes > 240)
    {
        if (closeTimeRes % 300) LogicError("closeTimeRes must be 5min factor while > 4min");
        closeTimeRes = 240 + closeTimeRes / 300;
    }
              ...
              beast::Journal j)
{
    ...
    if (isM(currency))
    {
        // M: return balance minus reserve
        if (amendmentRIPD1141 (view.info ().parentCloseTime))
        {
            ...
            JLOG (j.trace()) << "accountHolds:" <<
                ...
                " fullBalance=" << to_string (fullBalance.m()) <<
        ...
        else
        {
            ...
            auto const balance =
                    sle->getFieldAmount(sfBalance).m ();
            if (balance < reserve)
                amount.clear ();
            else
                amount = balance - reserve;
        ...
        beast::Journal j)
{
    ...
    {
        saFunds = saDefault;
        auto const limit = getIssueLimitAmount (saDefault.issue (), view);
        if (limit)
        {
            auto const total = getIssueTotalAmount (saDefault.issue (), view);
            if (total + saDefault > *limit)
            {
                saFunds = *limit - total;
            }
        }
...
{
    auto const sle = view.read(keylet::line(mAccount(), issuer, currency));
    if (sle && sle->isFieldPresent (sfTransferRate))
	return Rate { sle->getFieldU32 (sfTransferRate) };
    ...
    beast::Journal j)
{
    ...
    auto const sleMtchainState = std::make_shared<SLE>(
        ltMTCHAIN_STATE, uIndex);
    view.insert (sleMtchainState);
    ...
    if (tesSUCCESS == terResult)
    {
        ...
        sleMtchainState->setFieldU64 (sfLowNode, uLowNode);
        sleMtchainState->setFieldU64 (sfHighNode, uHighNode);

        sleMtchainState->setFieldAmount (
        ...
        if (bNoMTChain)
        {
            uFlags |= (bSetHigh ? lsfHighNoMtchain : lsfLowNoMtchain);
        }
        if (bFreeze)
        {
        ...
        if ((slePeer->getFlags() & lsfDefaultMtchain) == 0)
        {
            // The other side's default is no rippling
            uFlags |= (bSetHigh ? lsfLowNoMtchain : lsfHighNoMtchain);
        }

        sleMtchainState->setFieldU32 (sfFlags, uFlags);
        adjustOwnerCount(view, sleAccount, 1, j);

        // ONLY: Create mtchain balance.
        sleMtchainState->setFieldAmount (sfBalance, bSetHigh ? -saBalance : saBalance);
...
{
    // Detect legacy dirs.
    bool        bLowNode    = sleMtchainState->isFieldPresent (sfLowNode);
    bool        bHighNode   = sleMtchainState->isFieldPresent (sfHighNode);
    std::uint64_t uLowNode    = sleMtchainState->getFieldU64 (sfLowNode);
    std::uint64_t uHighNode   = sleMtchainState->getFieldU64 (sfHighNode);
    TER         terResult;

    JLOG (j.trace())
        << "trustDelete: Deleting mtchain line: low";
    ...
    {
        JLOG (j.trace())
                << "trustDelete: Deleting mtchain line: high";
        terResult   = dirDelete (view,
            false,
            uHighNode,
    ...
    JLOG (j.trace()) << "trustDelete: Deleting mtchain line: state";
    view.erase(sleMtchainState);
    ...
    beast::Journal j)
{
    ...
    auto const& currency = saAmount.getCurrency ();
    ...
    uint256 uIndex = getMtchainStateIndex (
        uSenderID, uReceiverID, saAmount.getCurrency ());
    auto sleMtchainState  = view.peek (keylet::line(uIndex));

    TER terResult;

    assert (!isM (uSenderID) && uSenderID != noAccount());
    assert (!isM (uReceiverID) && uReceiverID != noAccount());
    ...
    if (!sleMtchainState)
    {
        ...
        bool noMTChain = (sleAccount->getFlags() & lsfDefaultMtchain) == 0;
    ...
    {
        STAmount saBalance   = sleMtchainState->getFieldAmount (sfBalance);
        ...
        std::uint32_t const uFlags (sleMtchainState->getFieldU32 (sfFlags));
        bool bDelete = false;
        ...
        if (saBefore > zero
            ...
            && static_cast <bool> (uFlags & (!bSenderHigh ? lsfLowNoMtchain : lsfHighNoMtchain)) !=
               static_cast <bool> (view.read (keylet::account(uSenderID))->getFlags() & lsfDefaultMtchain)
            && !(uFlags & (!bSenderHigh ? lsfLowFreeze : lsfHighFreeze))
            && !sleMtchainState->getFieldAmount (
                !bSenderHigh ? sfLowLimit : sfHighLimit)
            // Sender trust limit is 0.
            && !sleMtchainState->getFieldU32 (
        ...
        sleMtchainState->setFieldAmount (sfBalance, saBalance);
        // ONLY: Adjust mtchain balance.
...
static
STAmount
mtchainTransferFee (ReadView const& view,
    ...
    beast::Journal j)
{
    if (from != issuer && to != issuer)
    {
        ...
        if (parityRate != rate)
        {
            ...
            JLOG (j.debug()) << "mtchainTransferFee:" <<
                " amount=" << amount.getFullText () <<
                " fee=" << fee.getFullText ();
...
static
TER
mtchainSend (ApplyView& view,
    AccountID const& uSenderID, AccountID const& uReceiverID,
    STAmount const& saAmount, STAmount& saActual, beast::Journal j)
{
    auto const issuer   = saAmount.getIssuer ();

    assert (!isM (uSenderID) && !isM (uReceiverID));
    assert (uSenderID != uReceiverID);

    if (uSenderID == issuer || uReceiverID == issuer || issuer == noAccount())
    {
        // Direct send: redeeming IOUs and/or sending own IOUs.
        mtchainCredit (view, uSenderID, uReceiverID, saAmount, false, j);
        saActual = saAmount;
        return tesSUCCESS;
    }
    ...
    if (!amendmentRIPD1141 (view.info ().parentCloseTime))
    {
        STAmount const saTransitFee = mtchainTransferFee (
    ...
    JLOG (j.debug()) << "mtchainSend> " <<
        to_string (uSenderID) <<
        " - > " << to_string (uReceiverID) <<
        " : deliver=" << saAmount.getFullText () <<
        " cost=" << saActual.getFullText ();

    TER terResult   = mtchainCredit (view, issuer, uReceiverID, saAmount, true, j);

    if (tesSUCCESS == terResult)
        terResult = mtchainCredit (view, uSenderID, issuer, saActual, true, j);
    ...
    STAmount const& saAmount, beast::Journal j)
{
    ...
    if (!saAmount.native ())
    {
        ...
        return mtchainSend (view, uSenderID, uReceiverID, saAmount, saActual, j);
    }
    ...
    /* M send which does not check reserve and can do pure adjustment.
    ...
    if (sender)
    {
        ...
        else
        {
            ...
            if (fv2Switch)
                view.creditHook (uSenderID, mAccount (), saAmount, sndBal);
    ...
    if (tesSUCCESS == terResult && receiver)
    {
        ...
        if (fv2Switch)
            view.creditHook (mAccount (), uReceiverID, saAmount, -rcvBal);
    ...
    beast::Journal j)
{
    ...
    if (before > zero
        ...
        // Sender reserve is set.
        && static_cast <bool> (flags & (!bSenderHigh ? lsfLowNoMtchain : lsfHighNoMtchain)) !=
           static_cast <bool> (sle->getFlags() & lsfDefaultMtchain)
...
{
    assert (!isM (account) && !isM (issue.account));
    ...
    uint256 const index = getMtchainStateIndex (
    ...
    if (!state)
    {
        ...
        bool noMTChain = (receiverAccount->getFlags() & lsfDefaultMtchain) == 0;

        return trustCreate (view, bSenderHigh, issue.account, account, index,
            receiverAccount, false, noMTChain, false, final_balance, limit, 0, 0, j);
    }
...
{
    if (amount.native()) return { tecNO_PERMISSION, false  };

    auto const& issue = amount.issue();
    auto k = keylet::line(mAccount(), issue);
    sleIssue = view.peek(k);
    if (sleIssue) return { tesSUCCESS, false };

    sleIssue = std::make_shared<SLE>(k);
    if (!sleIssue)
    {
        JLOG (j.error()) << "failed to create ledger entry for " << to_string(issue);
        return { tecNO_ENTRY, false };
    }

    TER ter;
    std::uint64_t uLowNode;
    std::tie (ter, std::ignore) = dirAdd (view, uLowNode, keylet::ownerDir (~issue.account),
              sleIssue->key(), describeOwnerDir(issue.account), j);

    if (ter != tesSUCCESS) return { ter, false };

    sleIssue->setFieldU64(sfLowNode, uLowNode);
    sleIssue->setFieldAmount (sfLowLimit,  amount);
    sleIssue->setFieldAmount (sfHighLimit, amount);

    view.insert(sleIssue);
    adjustOwnerCount(view, sleOwner, 1, j);
    return { ter, true };
}

#define MAX_ENTRY_NUM_PER_ACCOUNT 256
TER updateIssueTotalAmount (STAmount amount, ApplyView& view, beast::Journal j)
{
    ...
    auto r = addIssueInfo(amount, view, sleIssue, j);
    if (r.first != tesSUCCESS) return r.first;
    if (!r.second) view.update(sleIssue);
    */
    ...
    auto const& balance = sleIssue->getFieldAmount (sfBalance);
    if (!isM(balance))
    {
        amount += balance;
    }
    ...
    {
        forEachItem(view, issue.account, [&amount, &issue](std::shared_ptr<SLE const> const& sle)
        {
            if (sle && sle->getType () == ltMTCHAIN_STATE)
            {
                auto balance = sle->getFieldAmount (sfBalance);
                if (balance.getCurrency() == issue.currency)
                {
                    if (sle->getFieldAmount (sfLowLimit).getIssuer() == issue.account)
                    {
                        balance.negate();
                    }
    ...
    JLOG(j.warn()) << "update " << to_string(issue) << " issue total amount: "
                   << sleIssue->getFieldAmount(sfBalance).getText() << " -> " << amount.getText();
    sleIssue->setFieldAmount(sfBalance, amount);
...
{
    auto k = keylet::line(mAccount(), issue);
    auto const sleLimit = view.read(k);
    if (!sleLimit || !sleLimit->isFieldPresent(sfLimitAmount))
    {
        return boost::none;
    }

    return sleLimit->getFieldAmount(sfLimitAmount);
}
...
STAmount getIssueTotalAmount (Issue const& issue, ReadView const& view)
{
    ...
    if (sle)
    {
        auto const& amount = sle->getFieldAmount (sfBalance);
        if (!isM(amount)) return amount;
    }

    STAmount total(issue, 0);
    forEachItem(view, issue.account, [&total, &issue](std::shared_ptr<SLE const> const& sle)
    {
        if (sle && sle->getType () == ltMTCHAIN_STATE)
        {
            auto balance = sle->getFieldAmount (sfBalance);
...
STAmount getIssueTotalAmount (Issue const& issue, ApplyView& view, beast::Journal j)
{
    ...
    if (sle)
    {
        auto const& amount = sle->getFieldAmount (sfBalance);
        if (!isM(amount)) return amount;
        updateIssueTotalAmount (issue, view, j);
    }

    return getIssueTotalAmount (issue, view);
}
