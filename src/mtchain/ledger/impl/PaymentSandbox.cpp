        ...
        STAmount const& amount) const
{
    ...
    */
    ...
    if (isM(issuer) && adjustedAmt < beast::zero)
        // A calculated negative M balance is not an error case. Consider a
        // payment snippet that credits a large M amount and then debits the
            ...
            STAmount const& preCreditBalance)
{
    ...
    ApplyViewBase::creditHook (from, to, amount, preCreditBalance);
}
...
PaymentSandbox::balanceChanges (ReadView const& view) const
{
    ...
    // get the change in M balance, Account == root, issuer == root, currency == M
...
MAmount PaymentSandbox::mDestroyed () const
{
